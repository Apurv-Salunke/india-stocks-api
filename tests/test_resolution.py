...
def test_resolution():
    ...
    for s in niftys:
        print(f"  Token={s.token} Symbol='{s.symbol}' Exch='{s.exchange}' Type={s.instrument_type} TradeSym='{s.tradingsymbol}'")
    ...
    for s in opts:
        print(f"  Token={s.token} Symbol='{s.symbol}' Exch='{s.exchange}' Type={s.instrument_type} OptType={s.opt_type} Strike={s.strike} Expiry={s.expiry}")
    ...
    from india_stocks_api.instruments.database import InstrumentMaster
    sample = session.query(InstrumentMaster).filter(
