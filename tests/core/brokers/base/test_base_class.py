...
# from pandas import DateOffset, isna
import pytest
...
def test_json_dumps_simple_dict():
    ...
    data = {"key": "value"}
    result = Broker._json_dumps(data)
    assert result == '{"key": "value"}'
...
def test_json_dumps_non_dict_input():
    ...
    with pytest.raises(TypeError):
        Broker._json_dumps([1, 2, 3])  # List instead of dictionary


# # ----------------------------------------------------------------------------
# # Tests for Broker.totp_creator()
# # -----------------------------------------------------------------------------


def test_totp_creator_success():
    """
    ...
    totp = Broker.totp_creator(totpbase)
    assert len(totp) == 6
    assert totp.isdigit()


def test_totp_creator_invalid_base():
    """
    ...
    with pytest.raises(ValueError):
        Broker.totp_creator("invalid_base")


def test_totp_creator_empty_base():
    """
...
def test_valid_totp_base():
    ...
    assert Broker.totp_creator(valid_totp) is not None
...
def test_totp_base_lowercase():
    ...
    assert Broker.totp_creator(lowercase_totp) is not None
...
def test_totp_base_mixed_case():
    ...
    assert Broker.totp_creator(mixed_case_totp) is not None


# # ----------------------------------------------------------------------------
# # Tests for Broker.data_reader()
# # -----------------------------------------------------------------------------
...
# # ----------------------------------------------------------------------------
# # Tests for Broker.datetime_strp()
# # -----------------------------------------------------------------------------
...
# # ----------------------------------------------------------------------------
# # Tests for Broker.datetime_format()
# # ----------------------------------------------------------------------------


# def test_datetime_format_valid_input():
#     """
#     Test that Broker.datetime_format correctly formats a valid datetime object
#     using the specified format and returns a string.
#     """
#     dt_obj = datetime(2023, 5, 1, 14, 30, 0)
#     result = Broker.datetime_format(dt_obj, "%Y-%m-%d %H:%M:%S")
#     assert isinstance(result, str)
#     assert result == "2023-05-01 14:30:00"


# def test_datetime_format_different_format():
#     """
#     Test that Broker.datetime_format correctly formats a datetime object
#     using a different format and returns the correct string.
#     """
#     dt_obj = datetime(2023, 5, 1)
#     result = Broker.datetime_format(dt_obj, "%d/%m/%Y")
#     assert isinstance(result, str)
#     assert result == "01/05/2023"


# def test_datetime_format_with_microseconds():
#     """
#     Test that Broker.datetime_format correctly formats a datetime object
#     that includes microseconds and returns the correct string.
#     """
#     dt_obj = datetime(2023, 5, 1, 14, 30, 0, 123456)
#     result = Broker.datetime_format(dt_obj, "%Y-%m-%d %H:%M:%S.%f")
#     assert isinstance(result, str)
#     assert result == "2023-05-01 14:30:00.123456"


# def test_datetime_format_invalid_datetime():
#     """
#     Test that Broker.datetime_format raises a ValueError when given an invalid
#     datetime object (e.g., a string instead of a datetime object).
#     """
#     with pytest.raises(ValueError):
#         Broker.datetime_format("2023-05-01", "%Y-%m-%d %H:%M:%S")


# def test_datetime_format_invalid_format_string():
#     """
#     Test that Broker.datetime_format raises a ValueError when the format string
#     is invalid or contains unsupported specifiers.
#     """
#     dt_obj = datetime(2023, 5, 1, 14, 30, 0)
#     with pytest.raises(ValueError, match="Invalid format string"):
#         Broker.datetime_format(dt_obj, "%Y-%m-%d %Q")  # Invalid format specifier


# def test_datetime_format_empty_format():
#     """
#     Test that Broker.datetime_format returns an empty string when the format
#     string is empty.
#     """
#     dt_obj = datetime(2023, 5, 1, 14, 30, 0)
#     result = Broker.datetime_format(dt_obj, "")
#     assert result == ""


# def test_datetime_format_none_input():
#     """
#     Test that Broker.datetime_format raises a ValueError when None is passed
#     as the datetime object.
#     """
#     with pytest.raises(ValueError):
#         Broker.datetime_format(None, "%Y-%m-%d")


# def test_datetime_format_iso_format():
#     """
#     Test that Broker.datetime_format correctly formats a datetime object
#     using an ISO 8601 format string.
#     """
#     dt_obj = datetime(2023, 5, 1, 14, 30, 0, 123456)
#     result = Broker.datetime_format(dt_obj, "%Y-%m-%dT%H:%M:%S.%fZ")
#     assert isinstance(result, str)
#     assert result == "2023-05-01T14:30:00.123456Z"


# # ----------------------------------------------------------------------------
# # Tests for Broker.from_timestamp()
# # -----------------------------------------------------------------------------
...
# # ----------------------------------------------------------------------------
# # Tests for Broker.curent_datetime()
# # -----------------------------------------------------------------------------
...
# # ----------------------------------------------------------------------------
# # Tests for Broker.concatenate_dataframes()
# # -----------------------------------------------------------------------------
...
# # ----------------------------------------------------------------------------
# # Tests for Broker.filter_future_dates()
# # -----------------------------------------------------------------------------
...
# # ----------------------------------------------------------------------------
# # Tests for Broker.download_expiry_dates_nfo()
# # -----------------------------------------------------------------------------
...
def test_successful_request(mock_session):
    ...
    """
    # Generate expiry dates that are always in the future
    today = datetime.now()
    future_dates = [
    ...
    mock_response.json.return_value = {"records": {"expiryDates": future_dates}}
    ...
    print("expiry dates: ", Broker.expiry_dates)
...
def test_fallback_to_curl(mock_session, mock_popen, mock_sleep, use_curl):
    ...
    if use_curl:
        # Simulate the failure of req_session and fallback to popen (curl)
        mock_session.return_value.request.side_effect = Exception("Request failed")
        mock_popen.return_value.read.return_value = dumps(
            {"records": {"expiryDates": future_dates}}
        )

        print("Using curl fallback")
    else:
        # Simulate a successful request using req_session
        mock_session.return_value.request.return_value = mock_response
        print("Using req_session")

    # Test the download_expiry_dates_nfo method
    Broker.download_expiry_dates_nfo("BANKNIFTY")

    assert "BANKNIFTY" in Broker.expiry_dates
    assert len(Broker.expiry_dates["BANKNIFTY"]) == len(future_dates)
...
def test_all_attempts_fail(mock_popen, mock_session, mock_sleep):
    ...
    assert "BANKNIFTY" not in Broker.expiry_dates
...
# # ----------------------------------------------------------------------------
# # Tests for Broker.download_expiry_dates_bfo()
# # -----------------------------------------------------------------------------
...
# # ----------------------------------------------------------------------------
# # Tests for Broker.jsonify_expiry()
# # -----------------------------------------------------------------------------
...
#         Root.MIDCPNF: [
#             str(datetime.now().date() + timedelta(days=i)) for i in range(1, 4)
#         ],
#     }
...
#     }
#     assert set(result[WeeklyExpiry.CURRENT].keys()) == {
#         Root.BNF,
#         Root.NF,
#         Root.FNF,
#         Root.MIDCPNF,
...
#         Root.BANKEX,
#     }
#     assert set(result[WeeklyExpiry.EXPIRY].keys()) == {
#         Root.BNF,
#         Root.NF,
#         Root.FNF,
#         Root.MIDCPNF,
...
#         Root.BANKEX,
#     }
#     assert set(result[WeeklyExpiry.LOTSIZE].keys()) == {
#         Root.BNF,
#         Root.NF,
#         Root.FNF,
#         Root.MIDCPNF,
...
#             assert result[WeeklyExpiry.EXPIRY][root] == sorted(
#                 result[WeeklyExpiry.EXPIRY][root]
#             )
