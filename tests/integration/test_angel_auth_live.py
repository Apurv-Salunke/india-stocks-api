...
class TestAngelAuthLive:
    ...
    def _setup(self):
        creds = _get_creds()
        self.broker = AngelOne(**creds)
        self.creds = creds

    def test_authenticate_returns_true(self):
        assert self.broker.authenticate() is True

    def test_in_memory_tokens_set(self):
        self.broker.authenticate()
        assert get_auth_token() is not None
        assert get_feed_token() is not None

    def test_session_persisted_with_correct_keys(self):
        self.broker.authenticate()
    ...
    def test_no_secrets_on_disk(self):
        self.broker.authenticate()
        session = load_session("angel")
    ...
    def test_session_identity_matches(self):
        self.broker.authenticate()
        session = load_session("angel")

        assert session["api_key"] == self.creds["api_key"]
        assert session["client_code"] == self.creds["client_code"]
