            ...
            if token:
                try:
                    ...
                    if openalgo_symbol:
                        ...
                        logger.info(
                            f"Final token lookup: {symbol} -> {openalgo_symbol}"
                        )
                        continue
                except Exception as e:
...
def map_trade_data(trade_data):
    ...
    if isinstance(trade_data, dict):
        ...
        if "tradebook" in trade_data and isinstance(trade_data["tradebook"], list):
            logger.info(
                f"Using 'tradebook' field with {len(trade_data['tradebook'])} trades"
            )
...
def transform_tradebook_data(tradebook_data):
    ...
    if isinstance(tradebook_data, list) and tradebook_data:
        logger.info(
            f"Sample trade to transform: {json.dumps(tradebook_data[0], indent=2)[:500]}"
        )
    ...
    for trade in tradebook_data:
        ...
        if segment == "FNO" or any(
            marker in broker_symbol for marker in ["CE", "PE", "FUT"]
        ):
            exchange = "NFO"
        else:
            exchange = "NSE"
