...
def get_api_response(
    endpoint, auth_token, method="GET", params=None, data=None, debug=False
):
...
class BrokerData:
    def __init__(self, auth_token):
        ...
        self.time_constraints = [
            ...
            {"max_days": 365, "min_interval": "240"},  # 150-365 days: 240 min minimum
            {
                "max_days": 1080,
                "min_interval": "1440",
            },  # 365-1080 days: 1440 min minimum
            {
                "max_days": float("inf"),
                "min_interval": "10080",
            },  # >1080 days: 10080 min minimum
        ]
    ...
    def _convert_to_groww_params(self, symbol, exchange):
        ...
        if not exchange or exchange == symbol:
            exchange = "NSE"
            logger.info(
                f"Exchange not specified, defaulting to NSE for symbol {symbol}"
            )
        ...
        if exchange in ["NFO", "BFO"]:
            ...
            if br_symbol:
                ...
                trading_symbol = self._convert_openalgo_to_groww_derivative_symbol(
                    symbol
                )
                logger.debug(
                    f"Converted derivative symbol: {symbol} -> {trading_symbol}"
                )
        else:
    ...
    def fix_timestamps(self, df, interval):
        ...
        if interval in ["D", "1D", "1d", "W", "w", "1W", "1w"]:
            logger.info(
                f"Setting all {interval} candles to 09:15:00 IST market open time"
            )
            new_index = []
            for i, idx in enumerate(df.index):
                ...
                market_open = datetime(
                    date_part.year, date_part.month, date_part.day, 9, 15, 0
                )
        ...
        # For intraday: Ensure times are within market hours (9:15 AM - 3:30 PM)
            ...
            logger.info(
                "Ensuring intraday candles are within market hours (9:15 AM - 3:30 PM IST)"
            )
            ...
            for dt in df.index:
                ...
                market_close = datetime(
                    date_part.year, date_part.month, date_part.day, 15, 30, 0
                )
    ...
    def get_history(
        ...
        try:
            ...
            if timeframe in self.timeframe_map:
                ...
                logger.warning(
                    f"Unrecognized timeframe {timeframe}, defaulting to daily"
                )
            ...
            if is_weekly:
                chunk_size = (
                    300  # 300 days (about 43 weeks) per request for weekly data
                )
            elif is_daily:
            ...
            while current_start <= end_date:
                ...
                current_end = min(
                    current_start + timedelta(days=chunk_size - 1), end_date
                )
                ...
                if not chunk_candles or len(chunk_candles) == 0:
                    logger.warning(
                        f"No candles found for chunk {chunk_start} to {chunk_end}"
                    )
                    # Move to next chunk
                    current_start = current_end + timedelta(days=1)
                    continue
            ...
            if is_eod:  # Use the previously defined is_eod flag for consistency
                ...
                if isinstance(start_time, str):
                    ...
                    start_date = (
                        start_time if hasattr(start_time, "year") else start_time.date()
                    )
                else:
                ...
                if dates and rows:
                    ...
                    logger.info(
                        f"Created DataFrame with {len(df)} rows for daily timeframe"
                    )
                else:
                    df = pd.DataFrame()
                ...
                for candle in candles:
                    if isinstance(candle, list) and len(candle) >= 6:
                        ...
                        if "timestamp" in candle:
                            ...
                            if (
                                ts > 4102444800
                            ):  # If timestamp is likely in milliseconds
                                ts = ts / 1000  # Convert to seconds
                            # Create timezone-aware datetime in IST
                            dt = datetime.fromtimestamp(ts, tz=ist_tz)
                        else:
            ...
            if not df.empty:
                ...
                if not result_df.empty and "timestamp" in result_df.columns:
                    ...
                    logger.info(
                        f"First row timestamp: {sample_timestamp} ({sample_dt})"
                    )
            ...
            if not df.empty:
                    ...
                    if is_weekly and not df.empty:
                        ...
                        try:
                            ...
                            for dt in weekly_df.index:
                                ...
                                ist_tz = pytz.timezone("Asia/Kolkata")
                                market_open = datetime(
                                    dt.year, dt.month, dt.day, 9, 15, 0
                                )
                                market_open = ist_tz.localize(market_open)
                            ...
                            logger.info(
                                f"Manual method: creating {num_weeks} weekly candles"
                            )
                            ...
                            logger.info(
                                f"Manually created {len(weekly_df)} weekly candles"
                            )
                ...
                logger.info(
                    f"Final format - timestamp column values: {sample_timestamps}"
                )
                logger.info(
                    f"These represent market hours in IST: {', '.join(sample_times)}"
                )
    ...
    def get_valid_interval(
        self, start_time: str, end_time: str, requested_interval: str
    ) -> str:
        """
        ...
        if requested_interval not in self.timeframe_map:
            logger.warning(
                f"Unsupported interval: {requested_interval}, defaulting to 'D'"
            )
    ...
    def get_quotes(self, symbol_list, timeout: int = 5) -> Dict[str, Any]:
        ...
        # Handle plain string (like just "RELIANCE")
            ...
            if symbol.endswith("FUT") or symbol.endswith("CE") or symbol.endswith("PE"):
                ...
                logger.info(
                    f"Auto-detected derivative symbol: {symbol}, using NFO exchange"
                )
            else:
        ...
        for sym in symbol_list:
            try:
                ...
                if exchange == "NSE":
                    ...
                    logger.warning(
                        f"Unsupported exchange: {exchange}, defaulting to NSE"
                    )
                    groww_exchange = EXCHANGE_NSE
                    segment = SEGMENT_CASH
                ...
                try:
                    ...
                    if response and not response.get("error"):
                        logger.info(
                            f"Successfully retrieved quote for {symbol} on {exchange}"
                        )
                        ...
                        if response.get("status") == "SUCCESS" and isinstance(
                            ...
                            is_derivative = (
                                exchange in ["NFO", "BFO"] or segment == SEGMENT_FNO
                            )
                            ...
                            quote_item = {
                                ...
                                "total_buy_qty": safe_float(
                                    response.get("total_buy_quantity")
                                ),
                                "total_sell_qty": safe_float(
                            ...
                            logger.warning(
                                f"Invalid response format for {symbol} on {exchange}"
                            )
                            response = {}
                    else:
                        logger.warning(
                            f"Empty or error response for {symbol} on {exchange}"
                        )
                        response = {}
