...
def direct_get_order_book(auth):
    ...
    try:
        ...
        for segment in segments:
            ...
            while True:
                try:
                    ...
                    response = client.get(
                        GROWW_ORDER_LIST_URL, headers=headers, params=params
                    )
                    ...
                    if current_orders and page == 0:
                        ...
                        logger.debug(
                            f"Sample order fields: {list(sample_order.keys())}"
                        )
        ...
        logger.info(
            f"Successfully fetched total of {len(all_orders)} orders using direct API"
        )
        ...
        if all_orders:
            logger.info(
                f"First order sample: {json.dumps(all_orders[0], indent=2)[:500]}..."
            )
    ...
    try:
        ...
        for i, order in enumerate(orders):
            ...
            if order_id and is_executed:
                ...
                if (
                    ...
                    and "order" in order
                    and isinstance(order["order"], dict)
                ):
                ...
                for field in ["product", "product_type", "order_variety"]:
                    if field in order and order[field]:
                        ...
                        logger.info(
                            f"Found product type '{product_type}' in field '{field}'"
                        )
                        break
        ...
        for index, potential_order in enumerate(potential_trade_orders):
            ...
            if order_id.startswith("GLTFO"):
                segment = SEGMENT_FNO
                logger.info(
                    f"Using FNO segment for order {order_id} based on order ID prefix"
                )
            else:
                segment = segment_map.get(raw_segment, SEGMENT_CASH)
                logger.info(
                    f"Using segment {segment} for order {order_id} (from {raw_segment})"
                )
            ...
            try:
                ...
                if isinstance(trades_result, tuple) and len(trades_result) >= 1:
                    ...
                    if (
                        trades_data.get("status") == "success"
                        and "trades" in trades_data
                    ):
                        if trades_data["trades"]:
                            ...
                            logger.info(
                                f"Order {order_id} has no trades despite being executed"
                            )
                            ...
                            if potential_order.get("filled_quantity", 0) > 0:
                                ...
                                synthetic_trade = {
                                    ...
                                    "trade_date_time": datetime.now().strftime(
                                        "%Y-%m-%dT%H:%M:%S"
                                    ),
                                    "settlement_number": "",
                                ...
                                logger.info(
                                    f"Added synthetic trade for order {order_id}"
                                )
                    # Check for special cases: 404 errors for FNO orders
                        ...
                        if potential_order.get("filled_quantity", 0) > 0:
                            ...
                            all_trades.append(synthetic_trade)
                            logger.info(
                                f"Added synthetic FNO trade for order {order_id}"
                            )
                    else:
                        logger.warning(
                        ...
                        if potential_order.get(
                            "filled_quantity", 0
                        ) > 0 and potential_order.get("status", "").upper() in [
                            "EXECUTED",
                            "COMPLETE",
                            "FILLED",
                        ]:
                            ...
                            all_trades.append(synthetic_trade)
                            logger.info(
                                f"Added synthetic fallback trade for order {order_id}"
                            )
                else:
                    logger.warning(
                        f"Unexpected format for trades result for order {order_id}"
                    )
            except Exception as e:
        ...
        if all_trades:
            logger.info(
                f"Sample trade data: {json.dumps(all_trades[0], indent=2, default=str)}"
            )
        ...
        for trade in all_trades:
            ...
            openalgo_trade = {
                ...
                "tradingSymbol": trade.get(
                    "symbol", ""
                ),  # Capitalized for exact matching
...
def get_positions(auth):
    ...
    try:
        ...
        response_obj = client.get(
            positions_url, params=params, headers=headers, timeout=30
        )
        ...
        try:
            ...
            if (
                ...
                if (
                    "payload" in response_data
                    and "positions" in response_data["payload"]
                ):
                        ...
                        try:
                            ...
                            if db_symbol:
                                ...
                                if option_match:
                                    ...
                                    if future_match:
                                        # Extract components
                                        symbol_name, year, month_num, day = (
                                            future_match.groups()
                                        )
            ...
            try:
                ...
                if fno_response.status_code == 200:
                    ...
                    if (
                        ...
                        logger.info(
                            f"Found {len(fno_positions)} positions in FNO segment"
                        )
                            ...
                            if (
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        from openalgo.database.token_db import (
                                            get_oa_symbol,
                                        )
                                    ...
                                    logger.error(
                                        f"Error converting position symbol: {e}"
                                    )
                                    # Fall back to original symbol if conversion fails
        ...
        try:
            ...
            if response_obj.status_code == 200 and "payload" in response_data:
                ...
                if "holdings" in response_data["payload"]:
                    ...
                    for holding in raw_holdings:
                        transformed_holding = {
                            ...
                            "day_change_percentage": holding.get(
                ...
                error_message = response_data.get(
                    "message", "Error retrieving holdings"
                )
        ...
        for position in positions_list:
            ...
            if symbol_match and exchange_match and product_match:
                ...
                net_qty = str(
                    position.get(
                        "net_quantity",
                        position.get("netqty", position.get("quantity", "0")),
                    )
                )
...
def direct_place_order_api(data, auth):
    ...
    try:
        ...
        if db_record and db_record.brsymbol:
            ...
            logger.info(
                f"Using brsymbol from database: {original_symbol} -> {trading_symbol}"
            )
        else:
            # If not found in database, try format conversion as fallback
            trading_symbol = format_openalgo_to_groww_symbol(
        ...
        raw_id = re.sub(
            r"[^a-zA-Z0-9-]", "", raw_id
        )  # Remove non-alphanumeric/non-hyphen chars
        ...
        if hyphen_count > 2:
            ...
            for pos in positions[2:]:
                raw_id = (
                    raw_id[:pos] + "X" + raw_id[pos + 1 :]
                )  # Replace excess hyphens with 'X'
        ...
        if price is not None and order_type == ORDER_TYPE_LIMIT:
            ...
            try:
                ...
                logger.info(
                    f"Using price: {price_value} (original: {price}, type: {type(price)})"
                )
            except (ValueError, TypeError) as e:
                logger.error(
                    f"Invalid price value ({price}, type: {type(price)}): {str(e)}"
                )
                raise ValueError(
        ...
        try:
            ...
            logger.error(
                f"Invalid quantity value ({quantity}, type: {type(quantity)}): {str(e)}"
            )
            raise ValueError(
        ...
        logger.info(
            f"Placing {transaction_type} order for {quantity} of {trading_symbol}"
        )
        ...
        client = get_httpx_client()
        logger.info(
            f"Sending API request to {api_url} with payload: {json.dumps(payload)}"
        )
        ...
        if resp.status_code == 200:
            ...
            if response_data.get("status") == "SUCCESS":
                ...
                logger.error(
                    f"Order placement failed: {error_message}, Mode: {error_mode}"
                )
                logger.error(
                ...
                if "Invalid numeric value" in error_message:
                    logger.error(
                        "NUMERIC VALUE ERROR DETECTED - Debugging payload values:"
                    )
                    for field in [
                        "price",
                        "trigger_price",
            ...
            try:
                ...
                error_message = (
                    f"API error: {resp.status_code}. Raw response: {raw_response}"
                )
...
def place_smartorder_api(data, auth):
    ...
    try:
        ...
        logger.info(
            f"Target Position Size: {position_size} (type: {type(position_size)})"
        )
        ...
        if action:
            ...
            logger.info(
                f"This will move position from {current_position} to {position_size}"
            )
        ...
        for position in positions:
            try:
                ...
                trading_symbol = position.get(
                    "tradingsymbol",
                    position.get("trading_symbol", position.get("symbol")),
                )
                exchange = (
                ...
                if br_symbol:
                    ...
                    logger.warning(
                        f"No broker symbol found for {trading_symbol} in {exchange}"
                    )
                ...
                logger.info(
                    f"Square-off response: {api_response}, order_id: {order_id}"
                )
                ...
                elif api_response and api_response.get("message", "").startswith(
                    "API error: 400"
                ):
...
def cancel_order(orderid, auth, segment=None, symbol=None, exchange=None):
    ...
    try:
        ...
        if symbol and exchange:
            ...
            logger.info(
                f"Symbol conversion for cancel order: {symbol} -> {groww_symbol}"
            )
        ...
        is_fno_order = False
        if orderid.startswith("GLTFO") or any(
            x in orderid for x in ["CE", "PE", "FUT"]
        ):
            is_fno_order = True
            segment = SEGMENT_FNO
        ...
        if response_obj.status_code == 200:
            ...
            if isinstance(response_data, dict) and "payload" in response_data:
                ...
                if isinstance(payload, dict):
                    ...
                    if order_status == "CANCELLATION_REQUESTED":
                        ...
                        logger.info(
                            f"Order {orderid} has been cancelled (status: {order_status})"
                        )
                    else:
                        logger.info(
            ...
            logger.warning(
                f"Order cancellation failed with status {response_obj.status_code}"
            )
        ...
        if "CANCELLATION_REQUESTED" in str(e):
            logger.info(
                "Order seems to be in CANCELLATION_REQUESTED state despite exception"
            )
            response = {
                "status": "success",
                "orderid": orderid,
...
def direct_modify_order(data, auth):
    ...
    try:
        ...
        logger.info(
            f"Starting direct modify order process for order: {data.get('orderid')}"
        )
                    ...
                    for order in order_book_response["data"]:
                        if order.get("groww_order_id") == groww_order_id:
                            ...
                            if "order_type" in order:
                                ...
                                logger.info(
                                    f"Retrieved order type from order book: {order_type}"
                                )
                                break
            except Exception as e:
        ...
        if "quantity" in data:
            try:
                ...
                if quantity_value <= 0:
                    logger.warning(
                        f"Invalid quantity value: {quantity_value}. Must be positive."
                    )
                    raise ValueError(
                        f"Invalid quantity: {quantity_value}. Must be positive."
                    )
        ...
        if (
            ...
            try:
                ...
                if trigger_price_value <= 0:
                    logger.warning(
                        f"Trigger price should be positive: {trigger_price_value}"
                    )
                payload["trigger_price"] = trigger_price_value
                logger.info(
        ...
        logger.info(
            f"Modifying order {groww_order_id} with parameters: {json.dumps(payload)}"
        )
        ...
        if resp.status_code == 200:
            ...
            if "symbol" in data and data["symbol"]:
                ...
                logger.info(
                    f"Including OpenAlgo symbol in modify response: {data['symbol']}"
                )

            # Log the success
            logger.info(
                f"Successfully submitted modification for order {groww_order_id}"
            )
            ...
            try:
                ...
                if "Invalid numeric value" in error_message:
                    ...
                    # Additional debugging info about the request
                    logger.error(
                        f"Original modification data received: {json.dumps(data)}"
                    )
            except Exception as parse_error:
                error_message = (
                    f"API error: {resp.status_code}. Raw response: {raw_response}"
...
def modify_order(data, auth):
    ...
    if response_obj.status == 200:
        ...
        payload = (
            groww_response.get("payload", {})
            if isinstance(groww_response, dict)
            else {}
        )
        order_status = payload.get("order_status", "")
    ...
    try:
        ...
        if isinstance(order_book_result, tuple) and len(order_book_result) >= 1:
            ...
            if not orders and isinstance(order_response, list):
                ...
                logger.info(
                    f"Using order_response list directly, found {len(orders)} orders"
                )
        # Legacy handling for older SDK implementation
            ...
            if "data" in order_book_result and order_book_result["data"]:
                orders = order_book_result["data"]
                logger.info(
                    f"Found {len(orders)} orders in the order book (legacy format)"
                )
        # Direct handling if get_order_book returned a list
        elif isinstance(order_book_result, list):
            orders = order_book_result
            logger.info(
                f"Using order_book_result list directly, found {len(orders)} orders"
            )
        ...
        for i, order in enumerate(orders):
            ...
            order_status = order.get("order_status", order.get("status", ""))
            logger.info(
                f"Order {i+1}/{len(orders)} ID: {order_id}, Status: {order_status}"
            )
            ...
            if order_status.upper() in [s.upper() for s in cancellable_statuses]:
                try:
                    ...
                    if isinstance(cancel_result, tuple) and len(cancel_result) >= 1:
                        ...
                        cancel_response = (
                            cancel_result  # Direct assignment if not a tuple
                        )
                    ...
                    logger.error(
                        f"Error cancelling order {orderid if orderid else 'Unknown'}: {e}"
                    )
                    failed_to_cancel.append(
                        {
...
def get_order_trades(orderid, auth, segment=None):
    ...
    try:
        ...
        if segment is None:
            ...
            try:
                ...
                if isinstance(order_book_result, dict) and "data" in order_book_result:
                    ...
                    isinstance(order_book_result, tuple) and len(order_book_result) >= 1
                ):
                    order_book_data = order_book_result[0]
                ...
                if orderid.startswith("GMKFO") or orderid.startswith("GLTFO"):
                    ...
                    for order in order_data:
                        ...
                        if (
                            order.get("groww_order_id", order.get("orderid", ""))
                            == orderid
                        ):
                            ...
                            original_order_info["segment"] = order.get(
                                "segment", "UNKNOWN"
                            )
                            original_order_info["filled_quantity"] = order.get(
                                "filled_quantity", 0
                            )
        ...
        if segment is None:
            logger.warning(
                f"Could not determine segment for order {orderid}, defaulting to CASH"
            )
            segment = SEGMENT_CASH
        ...
        try:
            ...
            if (
                ...
                if (
                    "payload" in response_data
                    and "trade_list" in response_data["payload"]
                ):
                ...
                if (
                    ...
                    synthetic_trade = {
                        ...
                        "trade_date_time": datetime.datetime.now().strftime(
                            "%Y-%m-%dT%H:%M:%S"
                        ),
                        "settlement_number": "",
                    ...
                    logger.warning(
                        f"Error getting trades for order {orderid}: {error_message}"
                    )
                    if error_details:
                        logger.warning(
                            f"Error details: {json.dumps(error_details, indent=2)}"
            ...
            logger.error(
                f"Error parsing JSON response for trades for order {orderid}: {e}"
            )
        except Exception as e:
