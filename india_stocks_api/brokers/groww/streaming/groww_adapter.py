...
class GrowwWebSocketAdapter(BaseBrokerWebSocketAdapter):
        ...
        try:
            if not self.subscriptions:
                return self._create_success_response(
                    "No active subscriptions to unsubscribe"
                )
            ...
            for correlation_id, sub_info in subscriptions_copy.items():
                try:
                    ...
                    if response.get("status") == "success":
                        ...
                        self.logger.debug(
                            f"✅ Unsubscribed: {exchange}:{symbol} mode {mode}"
                        )
                    else:
                        failed_count += 1
                        failed_list.append(
            ...
            self.logger.info(
                "🔌 Calling disconnect() to terminate Groww connection completely..."
            )
            try:
                self.disconnect()
            ...
            self.logger.info(
                "✅ All subscriptions cleared and disconnected from Groww server"
            )
            self.logger.info(
                "✅ ZMQ resources cleaned up - no more data will be published"
            )
    ...
    def subscribe(
        ...
        if mode == 3 and depth_level != 5:
            self.logger.info(
                f"Groww only supports depth level 5, using 5 instead of {depth_level}"
            )
            depth_level = 5
            ...
            try:
                if mode in [1, 2]:  # LTP or Quote mode
                    ...
                    if instrumenttype == "INDEX" or "INDEX" in exchange:
                        ...
                        self.subscriptions[correlation_id]["mode"] = (
                            1  # Change to LTP mode
                        )
                    else:
                        ...
                        if "BSE" in groww_exchange:
                            self.logger.info(
                                f"🔴 BSE DEPTH subscription key: {sub_key}"
                            )
    ...
    def _resubscribe(self, correlation_id: str, sub_info: Dict):
        ...
        try:
            ...
            self.subscription_keys[correlation_id] = sub_key
            self.logger.info(
                f"Resubscribed to {sub_info['symbol']}.{sub_info['exchange']}"
            )
    ...
    def _on_data(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            has_market_data = any(
                key in data for key in ["ltp_data", "depth_data", "index_data"]
            )
            ...
            if "symbol" in data and "exchange" in data:
                ...
                symbol_from_data = data[
                    "symbol"
                ]  # This contains the actual symbol name now
                    ...
                    for cid, sub in self.subscriptions.items():
                        ...
                        is_index_match = (
                            ...
                            and (
                                ...
                                or (
                                    sub["exchange"] == "BSE_INDEX" and exchange == "BSE"
                                )
                            )
            ...
            if not subscription:
                ...
                if "BSE" in str(data) and "depth" in str(data).lower():
                    self.logger.error(
                        "🔴 BSE DEPTH DATA RECEIVED BUT NO SUBSCRIPTION FOUND!"
                    )
                    self.logger.error(f"   Data: {data}")
                    self.logger.error(f"   Active subscriptions: {self.subscriptions}")
                self.logger.warning(
                    f"Received data for unsubscribed token/symbol: {data}"
                )
                return
            ...
            if actual_mode == 1:  # LTP mode
                ...
                if (
                    ...
                    if market_data["ltp"] == 0:
                        self.logger.warning(
                            f"⚠️ NO VALID LTP DATA for {symbol}, check data source"
                        )
                    else:
                        self.logger.info(
                            f"📈 LTP recovered for {symbol}: {market_data['ltp']}"
                        )
                ...
                if "depth" not in market_data:
                    ...
                    self.logger.warning(
                        f"No depth data for {symbol}, creating empty structure"
                    )
            ...
            if "BSE" in exchange and mode == 3:
                ...
                if "depth" in market_data:
                    self.logger.info(
                        f"   Buy levels: {len(market_data['depth'].get('buy', []))}"
                    )
                    self.logger.info(
                        f"   Sell levels: {len(market_data['depth'].get('sell', []))}"
                    )
            ...
            self.logger.debug(
                f"✅ ZMQ Published: {topic} with {len(str(market_data))} bytes"
            )
    ...
    def _normalize_market_data(self, message: Dict, mode: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "ltt": message.get(
                "tsInMillis", message.get("timestamp", int(time.time() * 1000))
            ),
