...
def get_api_response(endpoint, auth, method="POST", payload=None, custom_timeout=None):
    ...
    try:
        ...
        if endpoint == "/timePriceSeries" or custom_timeout:
            ...
            timeout_value = (
                custom_timeout or 600
            )  # 10 minutes timeout for historical data
        ...
        if "timeout" in str(e).lower():
            ...
            raise Exception(
                "Request timeout - please try again with smaller date range"
            )
...
class BrokerData:
    ...
    def get_quotes(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            if not quote_data:
                logger.warning(
                    f"Empty quote data received for {br_symbol} on {firstock_exchange}"
                )
    ...
    def get_history_chunked(
        ...
        try:
            ...
            if max_days is None:
                if interval == "1m":
                    max_days = (
                        2  # Extra small chunks for 1-minute data to prevent timeouts
                    )
                elif interval in ["3m", "5m"]:
                    max_days = (
                        5  # Very small chunks for high-frequency data in notebooks
                    )
            ...
            logger.info(
                f"Using chunk size: {max_days} days (optimized for Jupyter notebooks)"
            )

            # If within limit, use regular method
            if total_days <= max_days:
                logger.info(
                    f"Date range within {max_days} day limit, using single request"
                )
            ...
            # Split into chunks
            logger.info(
                f"Date range exceeds {max_days} day limit, using chunked loading"
            )
            all_data = []
            ...
            while current_start <= end_dt:
                ...
                try:
                    ...
                    if not chunk_data.empty:
                        ...
                        print(
                            f"✅ Chunk {chunk_count}: Retrieved {len(chunk_data)} candles"
                        )
                    else:
            ...
            success_rate = (
                ((chunk_count - failed_chunks) / chunk_count) * 100
                if chunk_count > 0
                else 0
            )
            print(
    ...
    def get_history_intraday_chunks(
        ...
        try:
            logger.info(
                f"Using intraday chunking for 1m data from {start_date} to {end_date}"
            )
            ...
            while current_date <= end_dt:
                date_str = current_date.strftime(
                    "%d-%m-%Y"
                )  # Firstock uses DD-MM-YYYY format
                ...
                time_chunks = [
                    (
                        "00:00:00",
                        "23:59:59",
                    )  # Full day - let API determine available data
                ]
                ...
                for start_time, end_time in time_chunks:
                    try:
                        ...
                        br_symbol = get_br_symbol(symbol, exchange)
                        firstock_exchange = (
                            "NSE" if exchange == "NSE_INDEX" else exchange
                        )
                        ...
                        logger.info(
                            f"Fetching chunk: {start_time} to {end_time} on {date_str}"
                        )
                        ...
                        if response.get("status") == "success":
                            ...
                            if chunk_data:
                                ...
                                logger.info(
                                    f"Retrieved {len(chunk_data)} candles for chunk"
                                )
                        else:
                            logger.warning(
    ...
    def get_history(
        ...
        try:
            ...
            if interval == "1m":
                ...
                return self.get_history_intraday_chunks(
            ...
            while current_start <= end_dt:
                ...
                current_end = min(
                    current_start + timedelta(days=chunk_days - 1), end_dt
                )
                ...
                try:
                    ...
                    if not chunk_df.empty:
                        ...
                        successful_chunks += 1
                        logger.info(
                            f"✅ Chunk {chunk_count} successful: {len(chunk_df)} records"
                        )
                    else:
            ...
            success_rate = (
                (successful_chunks / chunk_count) * 100 if chunk_count > 0 else 0
            )
            logger.info(
                f"🎯 Chunked loading complete: {len(combined_df)} total records"
            )
            logger.info(
    ...
    def _get_single_history_chunk(
        ...
        try:
            ...
            logger.info(
                f"Getting {interval} data for {br_symbol} from {start_str} to {end_str}"
            )
            ...
            response = get_api_response(
                "/timePriceSeries", self.auth_token, payload=payload
            )
            ...
            for candle in response.get("data", []):
                try:
                    ...
                    if interval == "D":
                        ...
                        logger.info(
                            f"DEBUG: Daily candle timestamp: {timestamp} -> {debug_dt}"
                        )
            ...
            if df["timestamp"].dtype != "int64":
                logger.warning(
                    f"Timestamp dtype is {df['timestamp'].dtype}, converting to int64"
                )
            ...
            if interval == "D":
                ...
                df["timestamp"] = df["timestamp"].dt.normalize() + pd.Timedelta(
                    hours=9, minutes=15
                )
