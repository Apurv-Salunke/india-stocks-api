...
def close_all_positions(current_api_key, auth):
    ...
    if not positions_response or positions_response.get("status") != "success":
        return {
            ...
            "error": positions_response.get("error", {}).get(
        ...
        try:
            ...
            if not symbol:
                ...
                error_messages.append(
                    f"Failed to get symbol for token {position.get('token')}"
                )
                continue
            ...
            if response and response.get("status") == "success":
                ...
                error_messages.append(
                    f"Failed to close position for {symbol}: {error_msg}"
                )
    ...
    try:
        ...
        if response_data.get("status") == "success":
            ...
            error_msg = response_data.get("error", {}).get(
                "message"
            ) or response_data.get("message", "Failed to modify order")
            return {
                "status": "error",
                "message": error_msg,
                "code": response_data.get("code"),
