...
class ZebuWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        if api_key:
            ...
            self.actid = api_key
            self.logger.info(
                f"Using Zebu vendor code from BROKER_API_KEY: {self.actid}"
            )
        else:
            # Fallback to user_id if no API key is set
            self.actid = user_id
            self.logger.warning(
                f"No BROKER_API_KEY found. Using user_id '{user_id}' as actid."
            )
            self.logger.warning(
    ...
    def _on_close(self, ws, close_status_code, close_msg):
        ...
        self.logger.info(
            f"Zebu WebSocket connection closed: {close_status_code} - {close_msg}"
        )
        self.connected = False
