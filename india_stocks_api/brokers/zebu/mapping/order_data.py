...
def map_order_data(order_data):
    ...
    if order_data is None or (
        isinstance(order_data, dict) and (order_data["stat"] == "Not_Ok")
    ):
...
def map_trade_data(trade_data):
    ...
    if trade_data is None or (
        isinstance(trade_data, dict) and (trade_data["stat"] == "Not_Ok")
    ):
        # Handle the case where there is no data
        # For example, you might want to display a message to the user
        # or pass an empty list or dictionary to the template.
        ...
        for holding in holdings_data:
            ...
            for exch_tsym in nse_entries:
                transformed_position = {
                    ...
                    + max(
                        int(holding.get("npoadt1qty", 0)), int(holding.get("dpqty", 0))
                    ),
