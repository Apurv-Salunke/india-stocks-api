...
class DefinedGeWebSocket:
    ...
    def __init__(self, auth_data):
        ...
        if isinstance(auth_data, dict):
            ...
            self.susertoken = auth_data.get(
                "feed_token", ""
            )  # feed_token contains susertoken
    ...
    def _on_close(self, ws, close_status_code, close_msg):
        ...
        logger.info(
            f"DefinedGe WebSocket connection closed: {close_status_code} - {close_msg}"
        )
        self.connected = False
        self.authenticated = False
    ...
    def _handle_subscription_ack(self, data):
        ...
        if has_ohlc_in_ack:
            ...
            if now < market_open or now > market_close:
                ...
                logger.warning(
                    f"⚠️ Market open but NO OHLC in ACK for {exchange}|{token}"
                )
    ...
    def _handle_depth_ack(self, data):
        ...
        logger.info(
            f"Depth subscription acknowledged: {data.get('e')}|{data.get('tk')}"
        )
    ...
    def unsubscribe(self, subscription_type, tokens):
        ...
        try:
            ...
            with self.subscription_lock:
                ...
                logger.info(
                    f"Unsubscribe request sent for {subscription_type}: {subscription_key}"
                )
                return True
