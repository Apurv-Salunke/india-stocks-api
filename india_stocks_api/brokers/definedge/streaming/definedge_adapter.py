...
class MarketDataCache:
    ...
    def _log_cache_initialization(self, token: str, data: Dict) -> None:
        ...
        has_ohlc = any(
            data.get(f) and not self._is_zero_value(data.get(f))
            for f in ["o", "h", "l", "c"]
        )
        if has_ohlc:
            self.logger.info(
...
class DefinedgeWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        if not auth_data:
            ...
            feed_token = get_feed_token(
                user_id
            )  # This contains susertoken for DefinEdge
            ...
            if not definedge_uid:
                self.logger.error(
                    f"No DefinEdge user ID found in database for {user_id}"
                )
    ...
    def subscribe(
        ...
        if mode == 3:  # Depth mode
            if not DefinedgeCapabilityRegistry.is_depth_level_supported(
        ...
        if self.connected and self.ws_client:
            try:
                ...
                if self._should_ws_subscribe(scrip, subscription_type):
                    ...
                    self.logger.info(
                        f"[SUBSCRIBE] WebSocket subscription sent for {scrip}"
                    )
                else:
                    self.logger.info(
                ...
                if self._should_ws_unsubscribe(scrip, subscription["mode"]):
                    ...
                    if self.connected and self.ws_client:
                        try:
                            ...
                            success = self.ws_client.unsubscribe(
    ...
    def _on_data(self, wsapp, message) -> None:
        ...
        try:
            ...
            if message.get("t") == "tk":
                ...
                self.logger.info(
                    f"📸 Touchline ACK for {exchange}|{token} - Initial snapshot"
                )
                ...
                has_nonzero_ohlc = any(
                    v not in [None, "", "0", 0] for v in ohlc_values.values()
                )
                ...
                if has_nonzero_ohlc:
                    ...
                    self.logger.warning(
                        "⚠️ No OHLC in touchline ACK (market may be closed)"
                    )
            ...
            if not subscription:
                self.logger.warning(
                    f"Received data for unsubscribed token: {exchange}|{token}"
                )
                return
    ...
    def _publish_for_other_modes(
        ...
        try:
            ...
            with self.lock:
                for correlation_id, sub in self.subscriptions.items():
                    if sub["token"] == token and sub["mode"] in [
                        1,
                        2,
                    ]:  # LTP or Quote mode
                        mode = sub["mode"]
                        ...
                        if mode == 1:  # LTP mode - only send LTP
                            ...
                            self.logger.debug(
                                f"Published LTP data from depth for {symbol}"
                            )
    ...
    def _on_depth_data(self, wsapp, message) -> None:
        ...
        try:
            ...
            if message.get("t") == "df":
                ...
                if not hasattr(
                ...
                ) or not self._depth_ohlc_logged.get(f"{exchange}|{token}"):
                    ...
                    if has_ohlc:
                        ...
                        self.logger.warning(
                            f"✗ Depth feed has NO OHLC for {exchange}|{token}"
                        )
