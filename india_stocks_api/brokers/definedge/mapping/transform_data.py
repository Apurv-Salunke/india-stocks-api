...
def transform_modify_order_data(data, token_id):
    ...
    try:
        ...
        if not symbol:
            logger.error(
                f"Failed to get broker symbol for {data['symbol']} on {data['exchange']}"
            )
        ...
        if (
            ...
            try:
                ...
                logger.info(
                    f"Added trigger_price: {trigger_price} for pricetype: {pricetype}"
                )
        ...
        if (
            final_pricetype not in ["SL", "SL-M"]
            and "trigger_price" in transformed_data
        ):
            logger.warning(
                f"Removing trigger_price for non-SL order type: {final_pricetype}"
            )
