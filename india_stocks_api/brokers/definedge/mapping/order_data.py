...
def map_order_data(order_data):
    ...
    if isinstance(order_data, dict):
        if order_data.get("status") == "SUCCESS" and "orders" in order_data:
            ...
            logger.error(
                f"DefinedGe API error: {order_data.get('message', 'Unknown error')}"
            )
            return []
        else:
...
def transform_order_data(orders):
    ...
    for order in orders:
        if not isinstance(order, dict):
            logger.warning(
                f"Expected a dict, but found a {type(order)}. Skipping this item."
            )
            continue
        ...
        transformed_order = {
            ...
            "action": order.get(
                "order_type", ""
            ),  # DefinedGe uses 'order_type' for BUY/SELL
...
def map_trade_data(trade_data):
    ...
    if isinstance(trade_data, dict):
        if trade_data.get("status") == "SUCCESS" and "trades" in trade_data:
            ...
            logger.error(
                f"DefinedGe API error: {trade_data.get('message', 'Unknown error')}"
            )
            return []
        else:
            # Handle case where data might be directly in the response
...
def transform_tradebook_data(tradebook_data):
    ...
    for trade in tradebook_data:
        if not isinstance(trade, dict):
            logger.warning(
                f"Expected a dict, but found a {type(trade)}. Skipping this item."
            )
            continue
...
def map_position_data(position_data):
    ...
    if isinstance(position_data, dict):
        if position_data.get("status") == "SUCCESS" and "positions" in position_data:
            ...
            logger.error(
                f"DefinedGe API error: {position_data.get('message', 'Unknown error')}"
            )
            return []
        else:
            # Handle case where data might be directly in the response
...
def map_portfolio_data(portfolio_data):
    ...
    if isinstance(portfolio_data, dict):
        if portfolio_data.get("status") == "SUCCESS" and "data" in portfolio_data:
            ...
            logger.error(
                f"DefinedGe API error: {portfolio_data.get('message', 'Unknown error')}"
            )
            return {}
        else:
            # Handle case where data might be directly in the response
...
def transform_holdings_data(holdings_data):
    ...
    for holding in holdings:
        if not isinstance(holding, dict):
            logger.warning(
                f"Expected a dict, but found a {type(holding)}. Skipping this item."
            )
            continue
