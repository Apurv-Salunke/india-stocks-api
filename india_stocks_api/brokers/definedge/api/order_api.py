    ...
    try:
        ...
        if method.upper() == "GET":
            ...
            response = client.post(
                url, json=payload if payload else {}, headers=headers
            )
...
def get_open_position(tradingsymbol, exchange, product, auth):
    ...
    logger.info(
        f"Looking for: Symbol={tradingsymbol}, Exchange={exchange}, Product={product}"
    )
    ...
    if isinstance(positions_data, list):
        ...
        logger.info(
            f"Positions data is a direct list with {len(positions_list)} positions"
        )
        ...
        if (
            positions_data.get("stat") == "Ok"
            or positions_data.get("status") == "SUCCESS"
        ):
            # Definedge uses 'positions' key, not 'data'
            positions_list = positions_data.get(
            ...
            if positions_list:
                logger.info(
                    f"Using {len(positions_list)} positions despite missing status"
                )
    ...
    if positions_list:
        ...
        if net_qty == "0":
            logger.info(
                f"✗ No matching position found for {tradingsymbol} with product {product}"
            )
    else:
...
def place_order_api(data, auth):
    ...
    try:
        ...
        logger.info(
            f"Place order payload being sent to Definedge: {json.dumps(newdata, indent=2)}"
        )
        ...
        try:
            response_data = response.json()
            logger.info(
                f"Definedge API Parsed Response: {json.dumps(response_data, indent=2)}"
            )
...
def place_smartorder_api(data, auth):
    ...
    try:
        ...
        if position_size == 0 and current_position > 0:
            ...
            logger.info(
                "No position to square off (position_size=0, current_position=0)"
            )
            response_data = {
                "status": "success",
                "message": "No position to square off",
            }
            return res, response_data, orderid
...
def close_all_positions(current_api_key, auth):
    ...
    positions_data = positions_response.get(
        "data", positions_response.get("positions", [])
    )
    ...
    for position in positions_data:
        ...
        try:
            ...
            logger.warning(
                f"Invalid net quantity value: {netqty} for position: {position}"
            )
            continue
    ...
    for position in positions_to_close:
        try:
            ...
            if not symbol:
                logger.error(
                    f"Failed to get OpenAlgo symbol for {tradingsymbol} on {exchange}"
                )
...
def cancel_order(orderid, auth):
    ...
    try:
        ...
        if response_data.get("status") == "SUCCESS":
            ...
            return {
                "status": "success",
                "orderid": response_data.get("order_id", orderid),
            }, 200
        else:
...
def modify_order(data, auth):
    ...
    if data.get("stat") == "Ok" or data.get("status") == "SUCCESS":
        return {
            "status": "success",
            "orderid": data.get("order_id", data.get("norenordno")),
        }, 200
    else:
...
def cancel_all_orders_api(data, auth):
    ...
    logger.info(
        f"Cancel all orders request with strategy: {data.get('strategy', 'N/A')}"
    )
    ...
    orders_data = order_book_response.get(
        "data",
        order_book_response.get("orders", order_book_response.get("orderbook", [])),
    )
    ...
    orders_to_cancel = [
        ...
        or order.get("order_status", "").upper()
        in ["OPEN", "PENDING", "TRIGGER_PENDING"]
    ]
    ...
    for order in orders_to_cancel:
        ...
        orderid = (
            order.get("order_id") or order.get("norenordno") or order.get("orderid")
        )
