    ...
    def get_quotes(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            if response.get("status") != "SUCCESS":
                raise Exception(
                    f"API returned status: {response.get('status', 'Unknown')}"
                )
    ...
    def get_history(
        ...
        try:
            ...
            while current_start <= to_date:
                ...
                logger.debug(
                    f"Debug - Headers: Authorization key present: {bool(api_session_key)}"
                )
                ...
                try:
                    ...
                    logger.debug(
                        f"Debug - Total lines in CSV: {len(csv_data.split('\n'))}"
                    )
                    logger.debug(
                        f"Debug - Timeframe: {timeframe}, Interval: {interval}"
                    )
                    ...
                    logger.debug(
                        f"Debug - DataFrame shape after parsing: {chunk_df.shape}"
                    )
                    logger.debug(
                        f"Debug - DataFrame columns: {chunk_df.columns.tolist()}"
                    )
                    if not chunk_df.empty:
                        logger.debug(
                    ...
                    if chunk_df.empty:
                        ...
                        logger.info(
                            "Debug - This might be due to incorrect date parsing"
                        )
                        current_start = current_end + timedelta(days=1)
                        continue
            ...
            if not dfs:
                logger.debug(
                    "Debug - No data received from API, returning empty DataFrame"
                )
                return pd.DataFrame(
                    columns=[
                        "close",
            ...
            if "oi" not in df.columns:
                ...
                df["oi"] = (
                    pd.to_numeric(df["oi"], errors="coerce").fillna(0).astype(int)
                )
