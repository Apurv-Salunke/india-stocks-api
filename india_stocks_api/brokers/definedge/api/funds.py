...
def get_margin_data(auth_token):
    ...
    try:
        ...
        if response_data.get("status") == "SUCCESS" or "cash" in response_data:
            ...
            processed_margin_data["availablecash"] = format_value(
                response_data.get("cash", 0)
            )
            ...
            if unrealized_mtom == 0:
                ...
                unrealized_mtom = (
                    float(
                        response_data.get("currentUnrealizedMTOMDerivativeIntraday", 0)
                    )
                    + float(
                        response_data.get("currentUnrealizedMTOMDerivativeMargin", 0)
                    )
                    + float(response_data.get("currentUnrealizedMTOMEquityIntraday", 0))
                    + float(response_data.get("currentUnrealizedMTOMEquityMargin", 0))
                    + float(
                        response_data.get("currentUnrealizedMTOMCommodityIntraday", 0)
                    )
                    + float(
                        response_data.get("currentUnrealizedMTOMCommodityMargin", 0)
                    )
                )
