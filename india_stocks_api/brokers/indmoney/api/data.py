...
def get_api_response(endpoint, auth, method="GET", params=None):
    ...
    try:
        ...
        if len(res.text) < 5000:
            ...
            logger.info(
                f"Large response received ({len(res.text)} chars), logging summary only"
            )
    ...
    if (
        ...
        if "status" not in response or response_status != "success":
            response["status"] = "success"
            logger.info(
                "Added/corrected missing status field to successful data response"
            )
        return response
        ...
        try:
            ...
            try:
                ...
                if full_data and any(
                    key in full_data
                    for key in ["ltp", "live_price", "open", "high", "low"]
                ):
                    ...
                    if depth_levels and len(depth_levels) > 0:
                        ...
                        if "buy" in first_level:
                            result["bid"] = self._clean_number(
                                first_level["buy"].get("price", 0)
                            )
                        if "sell" in first_level:
                            result["ask"] = self._clean_number(
                                first_level["sell"].get("price", 0)
                            )
            ...
            try:
                ...
                logger.warning(
                    f"Could not fetch depth data for quotes: {str(depth_error)}"
                )

            # Build the final result
            result = {
                "ltp": self._clean_number(ltp_data.get("live_price", 0))
                if ltp_data
                else 0,
        ...
        try:
            ...
            for chunk_start, chunk_end in date_chunks:
                try:
                    ...
                    chunk_end_ts = self._date_to_timestamp_ms(
                        chunk_end, end_of_day=True
                    )
                    ...
                    response = get_api_response(
                        endpoint, self.auth_token, "GET", params
                    )
                    ...
                    logger.info(
                        f"Successfully processed {len(chunk_candles)} candles from chunk"
                    )
            ...
            if all_candles:
                ...
                logger.info(
                    f"Successfully fetched {len(df)} candles after deduplication"
                )
                logger.info(
