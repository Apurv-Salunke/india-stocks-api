...
def copy_from_dataframe(df):
    ...
    try:
        if filtered_data_dict:  # Proceed only if there's anything to insert
            ...
            logger.info(
                f"Bulk insert completed successfully with {total_inserted} new records."
            )
        else:
...
def reformat_symbol(row, file_segment=None):
    ...
    if instrument_name == "EQUITY":
        ...
        base_symbol = (
            trading_symbol.split("-")[0] if "-" in trading_symbol else trading_symbol
        )
...
def process_indmoney_csv(path):
    ...
    for segment in ["equity", "fno", "index"]:
        ...
        df["lotsize"] = (
            pd.to_numeric(df["LOT_UNITS"], errors="coerce").fillna(1).astype(int)
        )
    ...
    try:
        ...
        if not token_df.empty:
            ...
            return socketio.emit(
                ...
                {
                    "status": "success",
                    "message": "Successfully Downloaded Indmoney Instruments",
                },
            )
        else:
            return socketio.emit(
