    ...
    try:
        ...
        if isinstance(order_data, dict) and "status" in order_data:
            if order_data.get("status") in ["error", "failure"]:
                logger.error(
                    f"Error in order data: {order_data.get('message', 'Unknown error')}"
                )
                return []
        ...
        if isinstance(order_data, str):
            logger.error(
                f"Received string response instead of order data: {order_data[:200]}..."
            )
            return []
        ...
        if order_data:
            for order in order_data:
                ...
                # Map product types
                if (
                    order["exchangeSegment"] == "NSE"
                    or order["exchangeSegment"] == "BSE"
...
def transform_tradebook_data(tradebook_data):
    ...
    for trade in tradebook_data:
        transformed_trade = {
            ...
            "trade_value": trade.get("tradedQuantity", 0)
...
def map_position_data(position_data):
    ...
    try:
        ...
        for position in all_positions:
            ...
            position["lastTradedPrice"] = position.get(
                "avg_price", 0.0
            )  # Using avg_price as placeholder
            position["marketValue"] = position.get("net_qty", 0) * position.get(
                "avg_price", 0.0
            )
            position["pnlAbsolute"] = position.get("realized_profit", 0.0)
            position["multiplier"] = 1  # Default multiplier
            position["positionType"] = (
                "open" if position.get("net_qty", 0) != 0 else "closed"
            )
    ...
    try:
        ...
        for position in positions_data:
            ...
            transformed_position = {
                ...
                ),  # OpenAlgo expects string format
            }
...
def map_portfolio_data(portfolio_data):
    ...
    try:
        ...
        if not isinstance(portfolio_data, list):
            logger.warning(
                f"Expected list but got {type(portfolio_data)}: {portfolio_data}"
            )
            return []
        ...
        if portfolio_data:
            for holding in portfolio_data:
                ...
                holding["pnlAbsolute"] = (
                    0.0  # Will be calculated when we have market price
                )
                holding["pnlPercent"] = (
                    0.0  # Will be calculated when we have market price
                )
...
def calculate_portfolio_statistics(holdings_data):
    try:
        ...
        for holding in holdings_data:
            ...
            market_price = (
                avg_price  # Placeholder - should be replaced with live market data
            )
