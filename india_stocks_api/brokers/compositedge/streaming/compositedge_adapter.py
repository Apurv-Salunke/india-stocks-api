...
from .compositedge_mapping import (
    CompositedgeExchangeMapper,
    CompositedgeCapabilityRegistry,
)
...
class CompositedgeWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def __init__(self):
        ...
        self.logger.info(
            f"Compositedge adapter initialized with ZMQ port: {self.zmq_port}"
        )
    ...
    def initialize(
        ...
        self.logger.info(
            f"Using API Key: {api_key[:10]}... for Compositedge XTS connection"
        )
    ...
    def disconnect(self) -> None:
        ...
        self.logger.info(
            "*** DISCONNECT CALLED - Starting Compositedge disconnect process ***"
        )
    ...
    def subscribe(
        ...
        if mode == 3:  # Depth mode
            if not CompositedgeCapabilityRegistry.is_depth_level_supported(
    ...
    def unsubscribe(self, symbol: str, exchange: str, mode: int = 2) -> Dict[str, Any]:
        ...
        instruments = [
            {
                "exchangeSegment": CompositedgeExchangeMapper.get_exchange_type(
                    brexchange
                ),
    ...
    def _on_data(self, wsapp, message) -> None:
        ...
        try:
            self.logger.info(
                f"RAW COMPOSITEDGE DATA: Type: {type(message)}, Data: {message}"
            )
            self.logger.info(
