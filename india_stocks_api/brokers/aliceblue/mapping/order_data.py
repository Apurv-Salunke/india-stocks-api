...
def transform_order_data(orders):
    ...
    for order in orders:
        ...
        if "Trantype" not in order or "Prctype" not in order:
            logger.error(
                "Error: Missing required keys in the order. Skipping this item."
            )
            continue
...
def transform_tradebook_data(tradebook_data):
    ...
    for trade in tradebook_data:
        ...
        if average_price > 0:
            ...
            logger.warning(
                f"Zero or missing AvgPrice. Raw value: {trade.get('AvgPrice')}"
            )
        ...
        transformed_trade = {
            ...
            "timestamp": trade.get(
                "Filltime", ""
            ),  # Use Filltime which contains HH:MM:SS format
        }
...
def transform_positions_data(positions_data):
    ...
    for position in positions_data:
        ...
        if netqty != 0 and average_price > 0 and ltp > 0:
            ...
            pnlpercentage = (
                (pnl / (average_price * abs(netqty))) * 100 if average_price != 0 else 0
            )
        ...
        transformed_position = {
            ...
            "realised": round(
                float(position.get("realisedprofitloss", 0)), 2
            ),  # Realised P&L
...
def calculate_portfolio_statistics(holdings_data):
    ...
    try:
        totalholdingvalue = sum(
            float(item.get("Ltp", 0)) * int(item.get("HUqty", item.get("Holdqty", 0)))
            for item in holdings_data
        )
        totalinvvalue = sum(
            float(item.get("Price", 0)) * int(item.get("HUqty", item.get("Holdqty", 0)))
            for item in holdings_data
        )
        totalprofitandloss = sum(
            (float(item.get("Ltp", 0)) - float(item.get("Price", 0)))
            * int(item.get("HUqty", item.get("Holdqty", 0)))
            for item in holdings_data
        )

        for item in holdings_data:
            logger.info(
                f"Holdings item: LTP={item.get('Ltp')}, Price={item.get('Price')}, Qty={item.get('HUqty', item.get('Holdqty'))}"
            )
