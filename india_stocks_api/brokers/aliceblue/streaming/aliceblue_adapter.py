...
class AliceblueWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        try:
            if auth_data:
                ...
                self.logger.info(
                    f"From database: auth_token=[REDACTED], feed_token={feed_token}"
                )
                self.logger.info(
                    f"feed_token type: {type(feed_token)}, value: {repr(feed_token)}"
                )

                if not auth_token:
                    self.logger.error(
                        f"No authentication tokens found for user {user_id}"
                    )
                    raise ValueError(
                        f"No authentication tokens found for user {user_id}"
                    )
                ...
                if not broker_api_key:
                    self.logger.error(
                        "BROKER_API_KEY not found in environment variables"
                    )
                    raise ValueError(
                        "BROKER_API_KEY not found in environment variables"
                    )
            ...
            self.logger.info(
                f"AliceBlue WebSocket adapter initialized for user {user_id}"
            )
    ...
    def connect(self):
        ...
        try:
            ...
            self.logger.info(
                "Skipping WebSocket session creation - not required for authentication"
            )
            ...
            if success:
                ...
                return {
                    "success": False,
                    "error": "Failed to connect to AliceBlue WebSocket",
                }
            ...
            return {
                "success": False,
                "error": f"Error connecting to AliceBlue WebSocket: {e}",
            }
    ...
    def _authenticate_websocket(self, ws):
        ...
        try:
            ...
            if not self.session_id:
                self.logger.warning(
                    "No session_id (JWT) available, skipping authentication"
                )
                return
    ...
    def subscribe(
        ...
        try:
            ...
            if (
                not self.ws_client
                or not self.ws_client.sock
                or not self.ws_client.sock.connected
            ):
                self.logger.info(
                    "AliceBlue WebSocket not connected - attempting to reconnect..."
                )
            ...
            if exchange in ["NSE_INDEX", "BSE_INDEX", "MCX_INDEX"] and str(
                token
            ).startswith("999"):
                original_token = token
                token = str(token)[3:]  # Remove '999' prefix
                self.logger.info(
                    f"Adjusted index token from {original_token} to {token}"
                )

            # Determine feed type based on mode
            feed_type = (
                AliceBlueFeedType.DEPTH if mode == 3 else AliceBlueFeedType.MARKET_DATA
            )

            # Create subscription message
            sub_msg = self.message_mapper.create_subscription_message(
                ab_exchange, token, feed_type
            )
                ...
                with self.lock:
                    ...
                    if mode > existing_mode:
                        self.subscriptions[sub_key] = {
                            ...
                            "all_modes": self.subscriptions.get(sub_key, {}).get(
                                "all_modes", set()
                            )
                        ...
                        self.subscriptions[sub_key]["all_modes"] = self.subscriptions[
                            sub_key
                        ].get("all_modes", set()) | {mode}

                self.logger.info(
                    f"Subscribed to {symbol} ({ab_exchange}|{token}) for mode {mode}"
                )
    ...
    def _update_market_snapshot(
        self, symbol_key: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def unsubscribe(self, symbol: str, exchange: str, mode: int = 2) -> Dict[str, Any]:
        ...
        try:
            ...
            unsub_msg = self.message_mapper.create_unsubsciption_message(
                ab_exchange, token
            )
        ...
        try:
            ...
            if msg_type == "ck":
                ...
                if parsed_data.get("type") != "error":
                    ...
                    self.logger.error(
                        f"Error parsing tick data: {parsed_data['message']}"
                    )
                ...
                if parsed_data.get("type") != "error":
                    ...
                    self.logger.error(
                        f"Error parsing depth data: {parsed_data['message']}"
                    )
    ...
    def _resubscribe_all(self) -> None:
        ...
        for sub_key, sub_info in subscriptions_to_restore.items():
            try:
                self.logger.info(
                    f"Resubscribing to {sub_info['symbol']} on {sub_info['exchange']}"
                )
                self.subscribe(
                    sub_info["symbol"],
                    sub_info["exchange"],
    ...
    def _on_data_received(self, parsed_data):
        ...
        try:
            self.logger.debug(
                f"_on_data_received called with parsed_data: {parsed_data}"
            )
            ...
            self.logger.debug(
                f"Token type in data: {type(token)}, value: {repr(token)}"
            )
            self.logger.debug(
                f"Current subscriptions keys: {list(self.subscriptions.keys())}"
            )
            ...
            self.logger.debug(
                f"Final values for topic: exchange={exchange}, symbol={symbol}"
            )
            ...
            has_depth = (
                "bids" in parsed_data or "asks" in parsed_data or "depth" in parsed_data
            )
            has_quote = any(
                k in parsed_data for k in ["open", "high", "low", "close", "volume"]
            )
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            "supported_data_types": list(
                self.capability_registry.get_supported_data_types()
            ),
            "supported_exchanges": list(
                self.capability_registry.get_supported_exchanges()
            ),
