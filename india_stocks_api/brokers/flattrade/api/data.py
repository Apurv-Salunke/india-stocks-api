    ...
    def get_quotes(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            response = get_api_response(
                "/PiConnectTP/GetQuotes", self.auth_token, payload=payload
            )
    ...
    def get_history(
        ...
        try:
            ...
            for candle in response:
                ...
                try:
                    ...
                    if interval == "D":
                        ...
                        timestamp = int(
                            candle.get("ssboe", 0)
                        )  # Use ssboe for timestamp
                        data.append(
                            {
                                "timestamp": timestamp,
                                "open": float(
                                    candle.get("into", 0)
                                ),  # EOD uses 'into' for open
                                "high": float(
                                    candle.get("inth", 0)
                                ),  # EOD uses 'inth' for high
                                "low": float(
                                    candle.get("intl", 0)
                                ),  # EOD uses 'intl' for low
                                "close": float(
                                    candle.get("intc", 0)
                                ),  # EOD uses 'intc' for close
                                "volume": int(
            ...
            if interval == "D":
                ...
                utc_today = datetime.now().replace(
                    ...
                    if df.empty or df["timestamp"].max() < today_ts:
                        try:
                            ...
                            if quotes:
                                ...
                                # Append today's data
                                df = pd.concat(
                                    [df, pd.DataFrame([today_data])], ignore_index=True
                                )
                                logger.info(
