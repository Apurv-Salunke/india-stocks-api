...
def get_api_response(
    endpoint, auth, method="GET", payload="", max_retries=3, retry_delay=2
):
...
def get_open_positionss(tradingsymbol, exchange, product, auth):
    ...
    logger.debug(
        f"Calling get_token with symbol: {tradingsymbol}, exchange: {exchange}"
    )
    ...
    logger.debug(
        f"Initial Target Security ID (using exchange '{exchange}'): {target_security_id}"
    )
    ...
    logger.debug(
        f"Product: {product} (Broker format: {reverse_map_product_type(product)})"
    )
    ...
    if (
        ...
        for idx, position in enumerate(positions_data["data"], 1):
            pos_security_id = position.get(
                "security_id"
            )  # This is the token ID from Paytm API
            ...
            logger.debug(
                f"API Net Qty: {position.get('net_qty', position.get('netQty', '0'))}"
            )
            ...
            if pos_exchange == "NSE" and (
                "OPT" in position.get("instrument", "")
                or "FUT" in position.get("instrument", "")
            ):
                our_exchange = "NFO"
                logger.debug(
        ...
        logger.warning(
            f"No positions data available or error in API response: {positions_data}"
        )
...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    ...
    if positions_data and positions_data.get("status") and positions_data.get("data"):
        ...
        for position in positions_data["data"]:
            ...
            if pos_exchange == "NSE" and (
                "OPT" in pos_instrument
                or "FUT" in pos_instrument
                or pos_instrument == "OPTIDX"
            ):
                internal_exchange = "NFO"
            elif pos_exchange == "BSE" and (
                "OPT" in pos_instrument or "FUT" in pos_instrument
            ):
            ...
            logger.debug(
                f"Mapped to: internal_exchange={internal_exchange}, product={product}"
            )
            logger.debug(
...
def place_order_api(data, auth):
    ...
    res = type(
        "Response", (), {"status": 200 if response.get("status") == "success" else 500}
    )()
...
def close_all_positions(current_api_key, auth):
    ...
    if positions_response["status"] == "success":
        ...
        for position in positions_response["data"]:
            ...
            if int(net_qty) == 0:
                logger.info(
                    f"Skipping position with zero quantity: {position.get('security_id')}"
                )
                continue
    ...
    if successful_closes > 0 and failed_closes == 0:
        ...
        return {
            "status": "error",
            "message": f"Failed to close all {failed_closes} positions",
        }, 500
    else:
        ...
        if order["order_no"] == orderid:
            ...
            payload = {
                ...
                "product": reverse_map_product_type(
                    data.get("product", order["product"])
                ),
                "order_type": "MKT"
                if data.get("pricetype") == "MARKET"
