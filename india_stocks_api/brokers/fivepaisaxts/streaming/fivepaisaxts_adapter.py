...
from broker.fivepaisaxts.streaming.fivepaisaxts_websocket import (
...
class FivepaisaXTSWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def __init__(self):
        ...
        self.logger.info(
            f"Fivepaisa XTS adapter initialized with ZMQ port: {self.zmq_port}"
        )
    ...
    def initialize(
        ...
        if not auth_data:
            ...
            if not api_key or not api_secret:
                ...
                raise ValueError(
                    "Missing Fivepaisa XTS API credentials in environment variables"
                )
        ...
        self.logger.info(
            f"Using API Key: {api_key[:10]}... for Fivepaisa XTS connection"
        )
    ...
    def _extract_client_id_from_token(
        self, feed_token: str, fallback_user_id: str
    ) -> str:
        """
    ...
    def disconnect(self) -> None:
        ...
        self.logger.info(
            "*** DISCONNECT CALLED - Starting Fivepaisa XTS disconnect process ***"
        )
            ...
            if hasattr(self, "zmq_port"):
                with BaseBrokerWebSocketAdapter._port_lock:
                    if self.zmq_port in BaseBrokerWebSocketAdapter._bound_ports:
                        ...
                        self.logger.info(
                            f"Released port {self.zmq_port} from bound ports registry"
                        )
    ...
    def subscribe(
        ...
        if mode == 3:  # Depth mode
            if not FivepaisaXTSCapabilityRegistry.is_depth_level_supported(
        ...
        instruments = [
            {"exchangeSegment": exchange_type, "exchangeInstrumentID": token_str}
        ]

        self.logger.info(f"Final subscription request for {symbol}.{exchange}:")
        self.logger.info(
            f"  - Exchange Segment: {exchange_type} (type: {type(exchange_type)})"
        )
    ...
    def unsubscribe(self, symbol: str, exchange: str, mode: int = 2) -> Dict[str, Any]:
        ...
        instruments = [
            {
                "exchangeSegment": FivepaisaXTSExchangeMapper.get_exchange_type(
                    brexchange
                ),
        ...
        with self.lock:
            if correlation_id in self.subscriptions:
                ...
                self.logger.info(
                    f"Removed {symbol}.{exchange} from subscription registry"
                )
        ...
        if self.connected and self.ws_client:
            try:
                ...
                self.logger.info(
                    "Initiating disconnect and cleanup after unsubscription"
                )
                self.disconnect()
            ...
            self.logger.warning(
                "Not connected to XTS server, skipping unsubscribe request"
            )
            ...
            for correlation_id, sub in self.subscriptions.items():
                try:
                    self.ws_client.subscribe(
                        correlation_id, sub["mode"], sub["instruments"]
                    )
                    self.logger.info(
                        f"Resubscribed to {sub['symbol']}.{sub['exchange']}"
                    )
                except Exception as e:
                    self.logger.error(
    ...
    def _on_data(self, wsapp, message) -> None:
        ...
        try:
            self.logger.info(
                f"RAW FIVEPAISA DATA: Type: {type(message)}, Data: {message}"
            )
            self.logger.info(
    ...
    def _process_json_data(self, data: dict):
        ...
        try:
            ...
            self.logger.info(
                f"Mapped ExchangeSegment {exchange_segment} to exchange: {exchange}"
            )
            ...
            if not symbol:
                if exchange == "NSE" and not self._is_index_token(
                    token_str, exchange_segment
                ):
                ...
                elif exchange == "BSE" and not self._is_index_token(
                    token_str, exchange_segment
                ):
            ...
            self.logger.info(
                f"Publishing to topic: {topic} on ZMQ port: {self.zmq_port}"
            )
            ...
            self.logger.info(
                f"Environment ZMQ_PORT: {os.environ.get('ZMQ_PORT', 'Not Set')}"
            )
    ...
    def _normalize_market_data(
        self, message: Dict[str, Any], mode: int
    ) -> Dict[str, Any]:
        """
        ...
        if message_code == 1502 and "Touchline" in message:
            ...
            ltq = touchline.get(
                "LastTradedQunatity", touchline.get("LastTradedQuantity", 0)
            )
            ...
            ltq = message.get(
                "LastTradedQunatity", message.get("LastTradedQuantity", 0)
            )
            avg_price = message.get(
                "AveragePrice", message.get("AverageTradedPrice", 0)
            )
