...
def get_api_response(
    endpoint, auth, method="GET", payload="", feed_token=None, params=None
):
    AUTH_TOKEN = auth
    ...
    def _fetch_market_data(self, token: dict, message_code: int) -> dict:
        ...
        try:
            ...
            if not response or response.get("type") != "success":
                ...
                logger.warning(
                    f"Error fetching market data (code {message_code}): {error_msg}"
                )
                return None
            ...
            logger.error(
                f"Error in _fetch_market_data (code {message_code}): {str(e)}",
                exc_info=True,
            )
            return None
    ...
    def get_history(self, symbol, exchange, timeframe, from_date, to_date):
        ...
        try:
            ...
            while current_start <= to_date:
                ...
                if not raw_data:
                    logger.warning(
                        f"No data returned for period {from_str} to {to_str}"
                    )
            ...
            if compression_value == "D":
                ...
                final_df["timestamp"] = final_df["timestamp"] - pd.Timedelta(
                    hours=5, minutes=30
                )

                # Round to the proper interval based on market open (9:15 AM)
                interval_minutes = (
                    int(compression_value) // 60 if compression_value != "D" else 0
                )
                if interval_minutes > 0:
                    # Shift by 15 minutes to align with market open, round, then shift back
                    final_df["timestamp"] = (
                        final_df["timestamp"] - pd.Timedelta(minutes=15)
                    ).dt.ceil(f"{interval_minutes}min") + pd.Timedelta(minutes=15)
