...
class DhanWebSocket:
    ...
    def __init__(self, client_id: str, access_token: str, is_20_depth: bool = False):
        ...
        self.logger = logging.getLogger(
            f"dhan_websocket_{'20depth' if is_20_depth else '5depth'}"
        )
    ...
    def _handle_asyncio_compatibility(self):
        ...
        try:
            ...
            if platform.system() in ["Linux", "Darwin"]:  # Darwin is macOS
                ...
                self.logger.debug(
                    "Running on Windows, no asyncio compatibility adjustments needed"
                )
        ...
        for i in range(0, len(instruments), max_batch_size):
            ...
            try:
                if self.ws and hasattr(self.ws, "send") and callable(self.ws.send):
                    ...
                    self.logger.debug(
                        f"Subscribed to {len(batch)} instruments in {mode} mode"
                    )
                else:
                ...
                self.logger.error(
                    f"Error subscribing to instruments: {e}", exc_info=True
                )
                return False
    ...
    def _on_message(self, ws, message):
        ...
        try:
            ...
            if isinstance(message, (bytes, bytearray)):
                self.logger.debug(
                    f"Received binary message of length: {len(message)} bytes"
                )
                self._parse_binary_message(message)
            else:
                self.logger.warning(
                    f"Received non-binary message: {type(message)}: {message}"
                )
    ...
    def _parse_regular_message(self, data: bytes):
        ...
        while offset < len(data):
            ...
            if parsed_data and self.on_data:
                self.logger.debug(
                    f"Sending parsed data to callback: {parsed_data.get('type')}"
                )
    ...
    def _parse_20_depth_message(self, data: bytes):
        ...
        while offset < len(data):
            ...
            if feed_response_code in [41, 51]:  # 20-depth bid/ask
                ...
                self.logger.warning(
                    f"Unknown 20-depth response code: {feed_response_code}"
                )
    ...
    def _parse_full_packet(
        ...
        for i in range(5):
            ...
            ask_qty = struct.unpack(
                "<I", payload[packet_offset + 4 : packet_offset + 8]
            )[0]
            bid_orders = struct.unpack(
                "<H", payload[packet_offset + 8 : packet_offset + 10]
            )[0]
            ask_orders = struct.unpack(
                "<H", payload[packet_offset + 10 : packet_offset + 12]
            )[0]
            bid_price = struct.unpack(
                "<f", payload[packet_offset + 12 : packet_offset + 16]
            )[0]
            ask_price = struct.unpack(
                "<f", payload[packet_offset + 16 : packet_offset + 20]
            )[0]
    ...
    def _handle_disconnect_packet(self, payload: bytes):
        ...
        if len(payload) >= 2:
            ...
            self.logger.warning(
                f"Received disconnect packet with code: {disconnect_code}"
            )
