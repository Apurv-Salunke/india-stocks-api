...
def get_api_response(endpoint, auth, method="POST", payload=""):
    ...
    if response.get("status") == "failed":
        ...
        error_msg = error_mapping.get(
            error_code, f"Dhan API Error {error_code}: {error_message}"
        )
...
class BrokerData:
    ...
    def _convert_to_dhan_request(self, symbol, exchange):
        ...
        security_id = get_token(
            symbol, exchange
        )  # This should be mapped to Dhan's security ID
    ...
    def get_history(
        ...
        try:
            ...
            if not self._is_trading_day(start_date) and not self._is_trading_day(
                end_date
            ):
            ...
            if start_date == end_date:
                ...
                logger.info(
                    f"Start and end dates are same, increasing end date to: {end_date}"
                )
            ...
            if not security_id:
                raise Exception(
                    f"Could not find security ID for {symbol} on {exchange}"
                )
            logger.info(f"exchange: {exchange}")
            ...
            if interval == "D":
                ...
                for i in range(len(timestamps)):
                    ...
                    ist_timestamp = self._convert_timestamp_to_ist(
                        timestamps[i], is_daily=True
                    )
                    all_candles.append(
                        {
                ...
                if start_date == (end_dt - timedelta(days=1)).strftime("%Y-%m-%d"):
                    ...
                    for chunk_start, chunk_end in date_chunks:
                        ...
                        if not self._is_trading_day(
                            chunk_start
                        ) and not self._is_trading_day(chunk_end):
                            continue
            ...
            if interval == "D":
                ...
                if start_date <= today <= end_date:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error fetching today's data from quotes: {str(e)}"
                        )
    ...
    def get_quotes(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            logger.info(
                f"Mapped security_id: {security_id}, exchange_type: {exchange_type}"
            )

            payload = {
                exchange_type: [
                    int(security_id)
                ]  # Use the proper exchange type for indices
            }
