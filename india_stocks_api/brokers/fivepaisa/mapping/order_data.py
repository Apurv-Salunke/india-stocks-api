    ...
    if position_data:
        for position in position_data:
            ...
            if symbol_from_db:
                ...
                position["Exch"] = exchange
                if (
                    position["Exch"] == "NSE" or position["Exch"] == "BSE"
                ) and position["OrderFor"] == "D":
                    ...
                    position["Exch"] in ["NFO", "MCX", "BFO", "CDS"]
                    and position["OrderFor"] == "D"
                ):
...
def transform_holdings_data(holdings_data):
    transformed_data = []
    for holdings in holdings_data["Data"]:
        buyvalue = float(holdings.get("AvgRate", 0.0)) * float(
            holdings.get("Quantity", 0.0)
        )
        ltpvalue = float(holdings.get("CurrentPrice", 0.0)) * float(
            holdings.get("Quantity", 0.0)
        )
