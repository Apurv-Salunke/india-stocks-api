...
def get_positions(auth: str) -> Dict[str, Any]:
    ...
    while current_retry < max_retries:
        try:
            ...
            current_retry += 1
            logger.info(
                f"Timeout getting positions (attempt {current_retry}/{max_retries}): {e}"
            )
            if current_retry >= max_retries:
                logger.info(
                    "Maximum retries reached for positions data. Returning empty result."
                )
                return {
...
def get_open_position(
    ...
    try:
        ...
        if (
            ...
            logger.info(
                f"Found {len(positions_data['body']['NetPositionDetail'])} positions"
            )
        else:
...
def cancel_order(orderid: str, auth: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for order in orderbook_data["body"]["OrderBookDetail"]:
            ...
            if str(order["ExchOrderID"]) == str(orderid) or str(
                order["BrokerOrderId"]
            ) == str(orderid):
                order_details = order
                break
        ...
        if order_details["OrderStatus"] == "Pending" and (
            ...
            logger.info(
                "Order is in Pending status with no exchange ID yet. Cannot cancel."
            )
            return {
                "status": "error",
...
def modify_order(data: Dict[str, Any], auth: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not exchange_order_id:
            return {
                "status": "error",
                "message": "Exchange Order ID not found for this order",
            }, 400
        ...
        if result.get("head", {}).get("status") == "0":
            ...
            error_msg = result.get("head", {}).get(
                "statusDescription", "Failed to modify order"
            )
            return {"status": "error", "message": error_msg}, response.status_code
