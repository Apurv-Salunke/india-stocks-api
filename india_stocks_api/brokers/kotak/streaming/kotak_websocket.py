...
class KotakWebSocket:
    ...
    def _send(self, msg):
        with self._lock:
            if not self._is_connected or self.ws is None:
                logger.debug(
                    f"[KOTAK WSS QUEUE] Queuing message until connection open: {msg}"
                )
    ...
    def _handle_open(self):
        ...
        try:
            ...
            logger.debug(
                f"[KOTAK WSS SEND] Sending explicit connection request: {cn_msg}"
            )
    ...
    def _process_single_message(self, msg):
        ...
        elif "ltp" in msg or any(
            field in msg for field in ["bp", "sp", "op", "h", "lo", "c", "v"]
        ):
    ...
    def _is_partial_update(self, msg):
        ...
        if (
            ltp
            and not symbol_name
            and not (has_open or has_high or has_low or has_close)
        ):
            return True
    ...
    def _parse_depth_with_state(self, msg, symbol_key):
        ...
        with self._lock:
            ...
            if not has_price_data and symbol_key in self._symbol_state:
                ...
                for key, value in msg.items():
                    if key in [
                        ...
                        key not in ["tk", "e"] and value
                    ):  # Update other non-zero fields
                        stored_data[key] = value
            ...
            for i in range(5):
                ...
                bid_orders = int(
                    msg.get(bid_orders_key, 0)
                )  # Use actual bid order count
                ...
                ask_orders = int(
                    msg.get(ask_orders_key, 0)
                )  # Use actual ask order count
    ...
    def _parse_quote(self, msg):
        ...
        return self._parse_quote_with_state(
            msg, f"{msg.get('e', '')}|{msg.get('tk', '')}"
        )

    def _parse_depth(self, msg):
        """Legacy method - maintained for backward compatibility."""
        return self._parse_depth_with_state(
            msg, f"{msg.get('e', '')}|{msg.get('tk', '')}"
        )
