...
def process_kotak_nse_csv(path):
    ...
    df_filtered = filtereddataframe[
        filtereddataframe["pGroup"].isin(["EQ", "BE", ""])
    ].copy()
...
def combine_details(row):
    ...
    if row["instrumenttype"] == "FUT":
        ...
        row["strike"] = (
            int(row["strike"]) if row["strike"].is_integer() else row["strike"]
        )
...
def process_kotak_nfo_csv(path):
    ...
    tokensymbols["strike"] = tokensymbols["strike"].apply(
