...
class KotakWebSocket:
    ...
    def _send_messages(self, token, sid, exchange, scrip, message_type):
        try:
            ...
            self.ws.hs_send(
                json.dumps({"type": "cn", "Authorization": token, "Sid": sid})
            )
                ...
                if isinstance(msg, dict):
                    if "name" in msg and msg["name"] == "dp":  # Check for depth data
                        ...
                        for i in range(5):
                            ...
                            bids.append(
                                {"price": price, "quantity": int(msg.get(qty_key, 0))}
                            )
    ...
    def on_open(
        self, token, sid, exchange, scrip, message_type
    ):  # Add message_type parameter
        threading.Thread(
            target=self._send_messages, args=(token, sid, exchange, scrip, message_type)
        ).start()

    def connect(
        self, url, token, sid, exchange, scrip, message_type
    ):  # Add message_type parameter
...
class BrokerData:
    ...
    def get_quotes(self, symbol, exchange):
        try:
            ...
            ws = KotakWebSocket()
            ws.connect(
                self.ws_url, self.auth_token, self.sid, kotak_exchange, token, "mws"
            )  # Changed to "mws"
            time.sleep(0.2)
    ...
    def get_depth(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            ws = KotakWebSocket()
            ws.connect(
                self.ws_url, self.auth_token, self.sid, kotak_exchange, token, "dpsp"
            )
            time.sleep(0.2)
    ...
    def get_supported_intervals(self) -> dict:
        ...
        intervals = {
            "seconds": sorted(
                [k for k in self.timeframe_map.keys() if k.endswith("s")]
            ),
            "minutes": sorted(
                [k for k in self.timeframe_map.keys() if k.endswith("m")]
            ),
