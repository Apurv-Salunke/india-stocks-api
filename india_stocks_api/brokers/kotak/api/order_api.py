...
def get_api_response(endpoint, auth_token, method="GET", payload=""):
    ...
    conn.request(
        method, f"{endpoint}?" + urllib.parse.urlencode(query_params), payload, headers
    )
...
def get_holdings(auth_token):
    return get_api_response(
        "/Portfolio/1.0/portfolio/v1/holdings?alt=false", auth_token
    )
...
def place_order_api(data, auth_token):
    ...
    conn.request(
        "POST",
        "/Orders/2.0/quick/order/rule/ms/place?" + urllib.parse.urlencode(query_params),
        payload,
        headers,
    )
    try:
...
def cancel_order(orderid, auth_token):
    ...
    conn.request(
        "POST",
        "/Orders/2.0/quick/order/cancel?" + urllib.parse.urlencode(query_params),
        payload,
        headers,
    )
    try:
...
def modify_order(data, auth_token):
    ...
    conn.request(
        "POST",
        "/Orders/2.0/quick/order/vr/modify?" + urllib.parse.urlencode(query_params),
        payload,
        headers,
    )
    try:
