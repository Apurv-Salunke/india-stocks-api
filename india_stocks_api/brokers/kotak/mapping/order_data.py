...
def transform_tradebook_data(tradebook_data):
    ...
    for trade in tradebook_data:
        transformed_trade = {
            ...
            "trade_value": float(trade.get("fldQty", 0.0))
            * float(trade.get("avgPrc", 0.0)),
...
def transform_positions_data(positions_data):
    ...
    for position in positions_data:
        transformed_position = {
            ...
            "quantity": (
                int(position.get("flBuyQty", 0)) - int(position.get("flSellQty", 0))
            )
        ...
        if transformed_position["quantity"] > 0:
            transformed_position["average_price"] = round(
                float(position["buyAmt"]) / float(position["flBuyQty"]), 2
            )
        elif transformed_position["quantity"] < 0:
            transformed_position["average_price"] = round(
                float(position["sellAmt"]) / float(position["flSellQty"]), 2
            )
...
def transform_holdings_data(holdings_data):
    ...
    for holding in holdings_data:
        transformed_position = {
            ...
            "pnl": round(
                (
                    float(holding.get("mktValue", 0.0))
...
def map_portfolio_data(portfolio_data):
    ...
    for portfolio in holdings:
        ...
        if portfolio["instrumentType"] == "Equity":
            ...
            logger.info(
                "Kotak Portfolio - Product Value for Delivery Not Found or Changed."
            )
...
def calculate_portfolio_statistics(holdings_data):
    ...
    totalprofitandloss = sum(
        item["mktValue"] - item["holdingCost"] for item in holdings_data
    )
