...
def map_order_data(order_data):
    ...
    for order in orders:
        ...
        if symbol:
            ...
            logger.info(
                f"Symbol is empty for exchange {exchange}. Keeping original symbol."
            )
...
def transform_order_data(orders):
    ...
    if isinstance(orders, dict) and "data" in orders:
        ...
        if isinstance(orders["data"], list):
            ...
            logger.warning(
                "Warning: Unexpected data structure. Expected orders in data field."
            )
            orders = []
    ...
    for order in orders:
        ...
        transformed_order = {
            ...
            "timestamp": order.get(
                "order_entry_time", order.get("order_timestamp", "")
            ),
...
def map_trade_data(trade_data):
    ...
    for trade in trades:
        ...
        if "order_side" in processed_trade and not processed_trade.get(
            "transaction_type"
        ):
        ...
        if "trade_quantity" in processed_trade and not processed_trade.get(
            "fill_quantity"
        ):
        ...
        if "trade_time" in processed_trade and not processed_trade.get(
            "fill_timestamp"
        ):
...
def map_position_data(position_data):
    ...
    # Case 3: direct array of positions
        ...
        logger.debug(
            f"DEBUG - Using Case 3: direct array with {len(position_data)} positions"
        )
    ...
    # Case 4: Legacy structure with 'net' key
        ...
        positions = (
            position_data["data"]["net"]
            if position_data["data"]["net"] is not None
            else []
        )
    else:
        ...
        if isinstance(position_data, dict):
            ...
            if "data" in position_data:
                ...
                if isinstance(position_data["data"], dict):
                    logger.info(
                        f"DEBUG - Data dict keys: {position_data['data'].keys()}"
                    )
        return []
    ...
    for position in positions:
        ...
        if (
            ...
            processed_position["average_price"] = processed_position[
                "average_buy_price"
            ]
        elif (
...
def transform_positions_data(positions_data):
    ...
    for position in positions_data:
        ...
        average_price_formatted = "{:.2f}".format(
...
def transform_holdings_data(holdings_data):
    ...
    if holdings_data and isinstance(holdings_data, list) and len(holdings_data) > 0:
        ...
        if (
            ...
            and "symbol" in first_item
            and "product" in first_item
        ):
            logger.debug(
    ...
    for holding in holdings_data:
        ...
        if (
            ...
            if "trading_symbol" in holding["instrument_details"]:
                ...
                logger.debug(
                    f"DEBUG - Found symbol in instrument_details.symbol: {tradingsymbol}"
                )
        ...
        if not tradingsymbol:
            logger.warning(
                f"WARNING: Could not find trading symbol in holding: {holding}"
            )
            continue
        ...
        if tradingsymbol and exchange:
            ...
            if tradingsymbol.endswith("-EQ"):
                ...
                logger.debug(
                    f"DEBUG - Removed -EQ suffix, symbol is now: {tradingsymbol}"
                )
            ...
            try:
                ...
                if oa_symbol:
                    ...
                    logger.debug(
                        f"DEBUG - Converted to OpenAlgo symbol: {tradingsymbol}"
                    )
                else:
                    logger.debug(
        ...
        for field in ["quantity", "free_quantity", "qty"]:
            if field in holding and holding[field] is not None:
                try:
                    ...
                    logger.info(
                        f"DEBUG - Found quantity in '{field}' field: {quantity}"
                    )
                    break
        ...
        for field in ["average_price", "buy_avg", "avg_price", "buy_price"]:
            if field in holding and holding[field] is not None:
                try:
                    ...
                    logger.info(
                        f"DEBUG - Found average price in '{field}' field: {average_price}"
                    )
                    break
                except (ValueError, TypeError):
                    continue
        ...
        for field in ["last_price", "ltp", "current_price", "market_price"]:
            if field in holding and holding[field] is not None:
                try:
                    ...
                    logger.info(
                        f"DEBUG - Found last price in '{field}' field: {last_price}"
                    )
                    break
                except (ValueError, TypeError):
                    continue
        ...
        transformed_holding = {
            ...
            "close_price": float(
                holding.get("close_price") or holding.get("previous_close", 0.0)
            ),
        ...
        if transformed_holding["symbol"] is None:
            logger.warning(
                "WARNING: Symbol is still None after transformation. Using fallback."
            )
