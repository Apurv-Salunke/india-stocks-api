...
def get_order_book(auth):
    ...
    if completed_orders.get("status") == "error":
        logger.info(
            f"DEBUG - Error fetching completed orders: {completed_orders.get('message')}"
        )
    ...
    if pending_orders.get("status") == "error":
        logger.info(
            f"DEBUG - Error fetching pending orders: {pending_orders.get('message')}"
        )
        ...
        if info_response.get("status") == "success":
            ...
            if client_id and username:
                auth_obj = Auth.query.filter_by(
                    name=username, broker="pocketful"
                ).first()
                if auth_obj:
...
def fetch_orders(auth, client_id, order_type):
    ...
    try:
        ...
        logger.debug(
            f"DEBUG - Response status for {order_type} orders: {response.status_code}"
        )
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Invalid JSON response for {order_type} orders",
            }
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                logger.debug(
                    f"DEBUG - Trade data received: {json.dumps(trade_data, indent=2)}"
                )
                ...
                if trade_data.get("status") == "success" and "data" in trade_data:
                    if (
                        isinstance(trade_data["data"], dict)
...
def get_holdings(auth):
    ...
    if holdings_response.get("status") == "error":
        logger.info(
            f"DEBUG - Error fetching holdings: {holdings_response.get('message')}"
        )
    ...
    try:
        if isinstance(holdings_response, dict):
            ...
            if (
                ...
                logger.debug(
                    f"DEBUG - Found {len(holdings_data)} holdings in data.holdings path"
                )

            # Case 2: data -> array
            elif "data" in holdings_response and isinstance(
                holdings_response["data"], list
            ):
                holdings_data = holdings_response["data"]
                logger.debug(
                    f"DEBUG - Found {len(holdings_data)} holdings in data path (list)"
                )
            ...
            # Case 3: holdings -> array
                ...
                logger.debug(
                    f"DEBUG - Found {len(holdings_data)} holdings in holdings path"
                )

            # Case 4: data -> other field containing holdings
            elif "data" in holdings_response and isinstance(
                holdings_response["data"], dict
            ):
...
def place_order_api(data, auth_token):
    ...
    # If client_id not in database, try to get it from trading_info endpoint
    if not client_id:
        info_response = get_api_response(
            f"{BASE_URL}/api/v1/user/trading_info", auth_token
        )
        if info_response.get("status") == "success":
            client_id = info_response.get("data", {}).get("client_id")
    ...
    response_data = get_api_response(
        ORDER_ENDPOINT, auth_token, method="POST", payload=newdata
    )
...
def close_all_positions(current_api_key, auth):
    ...
    try:
        ...
        logger.debug(
            f"DEBUG - Making direct GET request to {endpoint} with params: {params}"
        )
        response = client.get(endpoint, headers=headers, params=params)

        if response.status_code != 200:
            logger.error(
                f"DEBUG - Error response: {response.status_code} - {response.text}"
            )
            return {
                "status": "error",
        ...
        for position in positions:
            try:
                ...
                if int(net_quantity) == 0:
                    logger.debug(
                        f"DEBUG - Skipping position {symbol} with zero quantity"
                    )
                    continue
                ...
                oa_symbol = (
                    get_oa_symbol(symbol, exchange) if symbol and exchange else symbol
                )
                ...
                place_order_payload = {
                    ...
                    "strategy": "Squareoff",
                    "symbol": oa_symbol
                    or symbol,  # Use OA symbol if available, otherwise original
                    "action": action,
                    "exchange": exchange,
                    "pricetype": "MARKET",
                    "product": reverse_map_product_type(
                        exchange, position.get("product", "MIS")
                    ),
                    "quantity": str(quantity),
                }

                logger.debug(
                    f"DEBUG - Placing order to close position: {place_order_payload}"
                )

                # Try to place the order
                try:
                    status, api_response, orderid = place_order_api(
                        place_order_payload, auth
                    )
...
def modify_order(data, auth):
    ...
    try:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": "Invalid JSON response: " + response.text,
            }, 400
...
def cancel_all_orders_api(data, auth):
    ...
    try:
        ...
        response = client.get(endpoint, headers=headers, params=params)
        logger.debug(
            f"DEBUG - Response status for pending orders: {response.status_code}"
        )
        ...
        response_data = response.json()
        logger.debug(
            f"DEBUG - pending orders response preview: {str(response_data)[:200]}..."
        )
        ...
        if response_data.get("status") == "success":
            ...
            if (
                ...
                logger.debug(
                    f"DEBUG - Found {len(pending_orders)} orders in data.orders structure"
                )
            # Case 2: Orders directly in data array
            elif "data" in response_data and isinstance(response_data["data"], list):
                pending_orders = response_data["data"]
                logger.debug(
                    f"DEBUG - Found {len(pending_orders)} orders in data array structure"
                )
        ...
        orders_to_cancel = [
            order
            for order in pending_orders
            if order.get("status", "").upper()
            in [s.upper() for s in valid_cancel_statuses]
