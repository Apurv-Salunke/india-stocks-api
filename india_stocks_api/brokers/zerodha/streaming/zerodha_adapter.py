...
class ZerodhaWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        try:
            if broker_name != self.broker_name:
                return {
                    "status": "error",
                    "message": f"Invalid broker name: {broker_name}",
                }
                ...
                if self.ws_client.start():
                    ...
                    if self.ws_client.wait_for_connection(timeout=15.0):
                        ...
                        if self.ws_client.running:
                            self.logger.warning(
                                "⚠️ Client started but connection timeout"
                            )
                            return {
                                "status": "success",
                    ...
                    return {
                        "status": "error",
                        "message": "Failed to start WebSocket client",
                    }
    ...
    def _process_batch_subscriptions(self):
        ...
        for mode, tokens in mode_groups.items():
            try:
                self.logger.info(
                    f"📦 Batch subscribing {len(tokens)} tokens in {mode} mode"
                )
    ...
    def subscribe(
        ...
        try:
            ...
            if not self.ws_client.is_connected():
                self.logger.warning(
                    "⚠️ WebSocket not connected, waiting for connection..."
                )
    ...
    def _generate_topic(
        self, symbol: str, subscription_exchange: str, mode_str: str
    ) -> str:
        """
        ...
        try:
            for tick in ticks:
                ...
                if transformed_tick:
                    ...
                    subscribed_modes = (
                        set()
                    )  # Track which modes this symbol is subscribed to
                    ...
                    if not subscription_exchange:
                        self.logger.warning(
                            f"No subscription info found for token: {token}"
                        )
                        continue
                    ...
                    if original_tick_mode == "full":
                        ...
                        depth_topic = self._generate_topic(
                            symbol, subscription_exchange, "DEPTH"
                        )
                        self.logger.debug(
                            f"📊 Publishing DEPTH data to topic: {depth_topic}"
                        )
                        ...
                        if 2 in subscribed_modes:
                            ...
                            self.logger.debug(
                                f"📊 Publishing QUOTE data to topic: {quote_topic}"
                            )
                        ...
                        if 1 in subscribed_modes:
                            ...
                            ltp_topic = self._generate_topic(
                                symbol, subscription_exchange, "LTP"
                            )
                            self.logger.debug(
                                f"📊 Publishing LTP data to topic: {ltp_topic}"
                            )
    ...
    def _transform_index_tick(
        self, tick: Dict, symbol: str, exchange: str, mode: str
    ) -> Dict:
        ...
        if mode == "ltp":
            ...
            transformed = {
                ...
                "timestamp": tick.get("timestamp", int(time.time() * 1000)),
                "volume": tick.get(
                    "volume_traded", tick.get("volume", 0)
                ),  # Even if 0 for index
    ...
    def _transform_regular_tick(
        ...
        if mode == "ltp":
            ...
            transformed = {
                ...
                "average_price": tick.get(
                    "average_traded_price", tick.get("average_price", 0)
                ),
    ...
    def disconnect(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f"Error during ZMQ cleanup after disconnect error: {zmq_err}"
                )
