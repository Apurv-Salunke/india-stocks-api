...
def process_zerodha_csv(path):
    ...
    df.loc[(df["segment"] == "INDICES") & (df["exchange"] == "NSE"), "exchange"] = (
        "NSE_INDEX"
    )
    df.loc[(df["segment"] == "INDICES") & (df["exchange"] == "BSE"), "exchange"] = (
        "BSE_INDEX"
    )
    df.loc[(df["segment"] == "INDICES") & (df["exchange"] == "MCX"), "exchange"] = (
        "MCX_INDEX"
    )
    df.loc[(df["segment"] == "INDICES") & (df["exchange"] == "CDS"), "exchange"] = (
        "CDS_INDEX"
    )
    ...
    df["token"] = (
        df["instrument_token"].astype(str) + "::::" + df["exchange_token"].astype(str)
    )
