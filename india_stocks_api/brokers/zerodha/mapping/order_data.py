...
def map_order_data(order_data):
    ...
    if order_data:
        for order in order_data:
            ...
            if symbol:
                order["tradingsymbol"] = get_oa_symbol(
                    brsymbol=symbol, exchange=exchange
                )
            else:
                logger.info(
...
def map_position_data(position_data):
    ...
    if position_data:
        for position in position_data:
            ...
            # Check if a symbol was found; if so, update the trading_symbol in the current order
            if symbol:
                position["tradingsymbol"] = get_oa_symbol(
                    brsymbol=symbol, exchange=exchange
                )
            else:
                logger.info(
    ...
    if portfolio_data:
        for portfolio in portfolio_data:
            if portfolio["product"] == "CNC":
                ...
                logger.info(
                    "Zerodha Portfolio - Product Value for Delivery Not Found or Changed."
                )
...
def calculate_portfolio_statistics(holdings_data):
    totalholdingvalue = sum(
        item["last_price"] * item["quantity"] for item in holdings_data
    )
    totalinvvalue = sum(
        item["average_price"] * item["quantity"] for item in holdings_data
    )
