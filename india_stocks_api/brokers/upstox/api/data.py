...
class BrokerData:
    ...
    def get_quotes(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            if not quote_data:
                raise Exception(
                    f"No data received for instrument key: {instrument_key}"
                )
            ...
            if not quote:
                raise Exception(
                    f"No quote data found for instrument key: {instrument_key}"
                )
            ...
            return {
                ...
                if prev_ohlc.get("close")
                else 0,
                "volume": int(live_ohlc.get("volume", 0))
                if live_ohlc.get("volume")
                else 0,
    ...
    def get_history(
        ...
        try:
            ...
            logger.debug(
                f"Using chunk size: {chunk_days} days for {unit}/{interval_value}"
            )
            ...
            while current_start <= to_date:
                ...
                try:
                    ...
                    if not chunk_df.empty:
                        ...
                        successful_chunks += 1
                        logger.debug(
                            f"Chunk {chunk_count}: Retrieved {len(chunk_df)} candles"
                        )
                    else:
            ...
            logger.info(
                f"Chunking complete: {successful_chunks}/{chunk_count} chunks successful"
            )
            ...
            logger.exception(
                f"Error fetching historical data for {symbol} on {exchange}"
            )
            raise
        ...
        try:
            ...
            if unit in ["minutes", "hours"] and end_date.date() == current_date.date():
                ...
                try:
                    ...
                    logger.debug(
                        f"Intraday response status: {intraday_response.get('status')}"
                    )

                    if intraday_response.get("status") == "success":
                        intraday_candles = intraday_response.get("data", {}).get(
                            "candles", []
                        )
                        logger.info(
                            f"Got {len(intraday_candles)} candles from intraday endpoint"
                        )

                        # Debug: Log sample raw candle data immediately
                        if intraday_candles:
                            logger.debug(
                                f"Sample intraday candle: {intraday_candles[0]}"
                            )
                        ...
                        logger.debug(
                            f"Intraday endpoint returned error: {intraday_response}"
                        )
            ...
            if not all_candles or start_date.date() < current_date.date():
                ...
                try:
                    historical_response = get_api_response(
                        historical_url, self.auth_token
                    )
                    logger.debug(
                        f"Historical response status: {historical_response.get('status')}"
                    )

                    if historical_response.get("status") == "success":
                        historical_candles = historical_response.get("data", {}).get(
                            "candles", []
                        )
                        logger.info(
                        ...
                        if historical_candles:
                            logger.debug(
                                f"Sample historical candle: {historical_candles[0]}"
                            )

                        all_candles.extend(historical_candles)
                        logger.info(
                            f"Total candles after historical: {len(all_candles)}"
                        )
                    else:
                        logger.debug(
                            f"Historical endpoint returned error: {historical_response}"
                        )
            ...
            if unit == "days" and interval == "D":
                ...
                if start_date.date() <= today <= end_date.date():
                    logger.info(
                        "Today is within date range, checking if today's data exists"
                    )
                        ...
                        for candle in all_candles:
                            try:
                                candle_date = pd.to_datetime(
                                    candle[0],
                                    unit="ms"
                                    if isinstance(candle[0], (int, float))
                                    else None,
                                ).date()
                                ...
                                if candle_date == today:
                                    today_found = True
                                    logger.debug(
                                        "Today's data found in historical candles"
                                    )
                                    break
                            except Exception as e:
                    ...
                    if not today_found:
                        ...
                        try:
                            ...
                            if quotes and quotes.get("ltp", 0) > 0:
                                ...
                                logger.info(
                                    "Added today's daily candle from quotes API"
                                )
                            else:
                            ...
                            logger.info(
                                f"Could not get today's data from quotes API: {e}"
                            )
            ...
            logger.info(
                f"Timestamp column types before conversion: {df['timestamp'].dtype}"
            )
            logger.debug(
                f"Sample timestamp values: {df['timestamp'].head(10).tolist()}"
            )
            logger.info(
                f"Unique timestamp types: {df['timestamp'].apply(type).unique()}"
            )
            ...
            try:
                ...
                logger.info(
                    f"Successfully converted {len(df)} timestamps to Unix timestamps"
                )
            except Exception as e:
                ...
                def convert_timestamp(ts):
                    try:
                        ...
                        logger.warning(
                            f"Failed to convert timestamp {ts} ({type(ts)}): {e}"
                        )
                        return None
                ...
                if dropped_count > 0:
                    logger.warning(
                        f"Dropped {dropped_count} rows with invalid timestamps"
                    )
    ...
    def _filter_candles_by_date(
        ...
        for candle in candles:
            ...
            if isinstance(candle_ts, str):
                ...
                if (
                    candle_ts < 1e12
                ):  # If less than year 2001 in milliseconds, assume seconds
                    candle_ts = candle_ts * 1000
            else:
                logger.warning(
                    f"Unknown timestamp format: {type(candle_ts)} - {candle_ts}"
                )
                continue
    ...
    def get_depth(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                v3_url = (
                    f"/market-quote/ohlc?instrument_key={encoded_symbol}&interval=1d"
                )
