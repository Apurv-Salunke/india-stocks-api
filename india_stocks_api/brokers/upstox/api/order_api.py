...
def get_open_position(tradingsymbol, exchange, product, auth):
    ...
    """
    logger.debug(
        f"Getting open position for {tradingsymbol} on {exchange} with product {product}"
    )
    try:
...
def place_order_api(data, auth):
    ...
    try:
        ...
        if not token:
            logger.error(
                f"Instrument token not found for {data['symbol']}. Cannot place order."
            )
            return (
                None,
                {"status": "error", "message": "Instrument token not found"},
                None,
            )
        ...
        if response_data.get("status") == "success":
            ...
            logger.error(
                f"Failed to place order: {error_msg} | Response: {response_data}"
            )
...
def place_smartorder_api(data, auth):
    ...
    try:
        ...
        if (
            ...
            logger.info(
                "No existing position and quantity is specified. Placing a new order."
            )
...
def close_all_positions(current_api_key, auth):
    ...
    try:
        ...
        if positions_response.get("status") != "success" or not positions_response.get(
            "data"
        ):
...
def modify_order(data, auth):
    ...
    try:
        ...
        response_data = get_api_response(
            "/v2/order/modify", auth, method="PUT", payload=payload
        )
        ...
        if response_data.get("status") == "success":
            ...
            logger.error(
                f"Failed to modify order: {error_msg} | Response: {response_data}"
            )
            return {"status": "error", "message": error_msg}, 400
