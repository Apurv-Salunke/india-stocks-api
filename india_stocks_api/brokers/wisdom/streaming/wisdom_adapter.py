...
class WisdomWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        if not auth_data:
            ...
            if not api_key or not api_secret:
                ...
                raise ValueError(
                    "Missing Wisdom XTS API credentials in environment variables"
                )
    ...
    def disconnect(self) -> None:
        ...
        self.logger.info(
            "*** DISCONNECT CALLED - Starting Wisdom disconnect process ***"
        )
    ...
    def subscribe(
        ...
        if mode == 3:  # Depth mode
            if not WisdomCapabilityRegistry.is_depth_level_supported(
