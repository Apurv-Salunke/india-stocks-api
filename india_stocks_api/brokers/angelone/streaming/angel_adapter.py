...
class AngelWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def subscribe(
        ...
        if mode == 3:  # Snap Quote mode (includes depth data)
            if not AngelCapabilityRegistry.is_depth_level_supported(
            ...
            for correlation_id, sub in self.subscriptions.items():
                try:
                    self.ws_client.subscribe(
                        correlation_id, sub["mode"], sub["token_list"]
                    )
                    self.logger.info(
    ...
    def _on_data(self, wsapp, message) -> None:
        ...
        try:
            ...
            if not isinstance(message, dict):
                self.logger.warning(
                    f"Received message is not a dictionary: {type(message)}"
                )
                return
    ...
    def _extract_depth_data(self, message, is_buy: bool) -> List[Dict[str, Any]]:
        ...
        self.logger.debug(
            f"Extracting {side_label} depth data from message: {message.keys()}"
        )
        ...
        # For MCX, the data might be in a different format, check for best_five_buy/sell_market_data
            ...
            self.logger.debug(
                f"Found {side_label} depth data using best_five_buy_market_data: {len(depth_data)} levels"
            )
            ...
            self.logger.debug(
                f"Found {side_label} depth data using best_five_sell_market_data: {len(depth_data)} levels"
            )
