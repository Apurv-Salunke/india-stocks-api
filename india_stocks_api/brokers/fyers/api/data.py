...
class BrokerData:
        ...
        try:
            ...
            while current_start <= end_dt:
                try:
                    ...
                    current_end = min(
                        current_start + pd.Timedelta(days=chunk_days - 1), end_dt
                    )
                    ...
                    if candles:
                        ...
                        logger.debug(
                            f"No data available for period {chunk_start} to {chunk_end}"
                        )
                    ...
                    logger.error(
                        f"Error fetching chunk {chunk_start} to {chunk_end}: {e}"
                    )
                    if retry_count < max_retries:
                        retry_count += 1
                        logger.debug(
    ...
    def get_depth(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            if not depth_data:
                logger.warning(
                    f"No market depth data found for {br_symbol} in API response."
                )
                return {}
            ...
            bids_formatted = [
                {"price": b["price"], "quantity": b["volume"]} for b in bids[:5]
            ]
            asks_formatted = [
                {"price": a["price"], "quantity": a["volume"]} for a in asks[:5]
            ]
