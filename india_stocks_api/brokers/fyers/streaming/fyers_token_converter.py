...
class FyersTokenConverter:
        ...
        try:
            for symbol, exchange in symbol_exchange_pairs:
                ...
                if brsymbol:
                    ...
                    self.logger.error(
                        f"No brsymbol found in database for {symbol}@{exchange}"
                    )
        ...
        try:
            ...
            for symbol, exchange in symbol_exchange_pairs:
                if (symbol, exchange) in brsymbol_map:
                    ...
                    self.logger.error(
                        f"Symbol not found in database: {symbol}@{exchange}"
                    )
            ...
            return (
                [],
                {},
                [f"{info['symbol']}@{info['exchange']}" for info in symbol_info_list],
            )
        ...
        try:
            ...
            if brsymbols:
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Fyers API response for all symbols: {response_data}"
                    )
                    ...
                    if response_data.get("s") == "ok":
                        ...
                        for symbol, fytoken in valid_symbols.items():
                            hsm_token = self._convert_to_hsm_token(
                                symbol, fytoken, data_type
                            )
                            if hsm_token:
            ...
            if not hsm_tokens and remaining_symbols:
                ...
                fallback_tokens, fallback_mappings, fallback_invalid = (
                    self._manual_conversion(remaining_symbols, data_type)
                )
    ...
    def _convert_to_hsm_token(
        self, symbol: str, fytoken: str, data_type: str
    ) -> Optional[str]:
        """
    ...
    def _get_segment_from_exchange(
        self, exchange: str, symbol_name: str
    ) -> Optional[str]:
        """
        ...
        if exchange == "NSE":
            if symbol_name.endswith("-INDEX"):
                ...
                (
                    symbol_name.endswith("CE")
                    and any(char.isdigit() for char in symbol_name)
                )
                or (
                    symbol_name.endswith("PE")
                    and any(char.isdigit() for char in symbol_name)
                )
                or
    ...
    def convert_openalgo_to_fyers_symbol(self, exchange: str, symbol: str) -> str:
        ...
        if exchange == "BSE" and not symbol.endswith(("-A", "-B")):
            ...
            if not any(
                suffix in symbol for suffix in ["-INDEX", "FUT", "CE", "PE", "-EQ"]
            ):
