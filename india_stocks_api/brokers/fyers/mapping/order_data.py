...
def map_order_data(order_data):
    ...
    for order in order_list:
        ...
        if symbol:
            ...
            logger.warning(
                f"Symbol not found in order: {order}. Keeping original trading symbol."
            )
...
def transform_order_data(orders):
    ...
    for order in orders:
        if not isinstance(order, dict):
            logger.warning(
                f"Expected a dict, but found {type(order)}. Skipping this item."
            )
            continue
        ...
        if action == "unknown":
            logger.warning(
                f"Unknown side code '{side_code}' for order: {order.get('id')}"
            )
        ...
        if ordertype == "unknown":
            logger.warning(
                f"Unknown order type code '{type_code}' for order: {order.get('id')}"
            )
        ...
        if producttype == "unknown":
            logger.warning(
                f"Unknown product type '{product_code}' for order: {order.get('id')}"
            )
...
def map_trade_data(trade_data):
    ...
    for trade in trade_list:
        ...
        if symbol:
            ...
            logger.warning(
                f"Symbol not found in trade: {trade}. Keeping original trading symbol."
            )
...
def transform_tradebook_data(tradebook_data):
    ...
    for trade in tradebook_data:
        ...
        if action == "unknown":
            logger.warning(
                f"Unknown side code '{side_code}' for trade: {trade.get('orderNumber')}"
            )
