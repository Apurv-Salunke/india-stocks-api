...
class TradejiniWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def subscribe(
        ...
        if mode == 3:  # Depth mode
            if not TradejiniCapabilityRegistry.is_depth_level_supported(
    ...
    def _on_connection_event(self, ws, message) -> None:
        ...
        if status == "connected":
            ...
            self.logger.info(
                f"Tradejini WebSocket connection closed: {message.get('reason')}"
            )
    ...
    def _on_data(self, ws, message) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"RAW TRADEJINI DATA: Type={type(message)}, Data={message}"
            )
            ...
            if msg_type == "auth":
                ...
                if auth_status == 1:
                    self.logger.info(
                        "Successfully authenticated with Tradejini WebSocket"
                    )
                else:
                    self.logger.error(
                        f"Authentication failed with Tradejini WebSocket: {message}"
                    )
                return
            ...
            if msg_type == "EVENTS":
                self.logger.info(
                    f"Received event message: {message.get('message', '')}"
                )
                return
            ...
            for subscription in matching_subscriptions:
                ...
                if subscription_mode == 1:
                    ...
                    self.logger.warning(
                        f"Unknown subscription mode: {subscription_mode}"
                    )
                    continue
