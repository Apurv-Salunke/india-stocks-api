...
def get_api_response(endpoint, auth, method="GET", data=None, params=None):
    ...
    try:
        ...
        if response.status_code == 404:
            logger.warning(
                "get_api_response - API endpoint not found. Trying without /v2 prefix"
            )
            if method == "GET":
                response = client.get(
            ...
            logger.debug(
                f"get_api_response - Second attempt status: {response.status_code}"
            )
...
def get_trade_book(auth):
    ...
    try:
        ...
        if not isinstance(response_data, dict) or "s" not in response_data:
            logger.error(
                f"get_trade_book - Invalid API response format: {response_data}"
            )
            return {
                "status": "error",
                "data": [],
        ...
        if response_data["s"] != "ok":
            error_msg = (
                f"API error: {response_data.get('d', {}).get('msg', 'Unknown error')}"
            )
        ...
        for trade in trades_data:
            try:
                ...
                logger.debug(
                    f"get_trade_book - Transformed trade: {transformed_trade['orderid']}"
                )
            ...
            for position in positions:
                try:
                    ...
                    transformed_position = {
                        ...
                        "quantity": net_qty,
                        "average_price": str(
                            round(float(position.get("netAvgPrice", 0.0)), 2)
                        ),
                    }

                    logger.debug(
                        f"Position transformed: {tradingsymbol} → {openalgo_symbol}"
                    )
                    ...
                    logger.error(
                        f"Error transforming position: {str(e)}", exc_info=True
                    )
                    continue
...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    ...
    try:
        ...
        if not positions_response or not isinstance(positions_response, dict):
            logger.error(
                f"get_open_position - Invalid positions response: {positions_response}"
            )
            return "0"
        ...
        for position in positions:
            try:
                ...
                pos_trd_sym = (
                    str(sym_data.get("trdSym", "")).upper().strip()
                )  # e.g., 'YESBANK-EQ'
                pos_sym = (
                    str(sym_data.get("sym", "")).upper().strip()
                )  # e.g., 'YESBANK'
                ...
                try:
                    ...
                    logger.warning(
                        f"get_open_position - Invalid netQty {position.get('netQty')}"
                    )
                    continue
...
def place_order_api(data, auth):
    ...
    try:
        ...
        AUTH_TOKEN = auth
        logger.info(
            f"place_order_api - Placing order for {data['symbol']} on {data['exchange']}"
        )
        ...
        try:
            ...
            logger.debug(
                f"place_order_api - Response headers: {dict(response.headers)}"
            )
            ...
            if response_data.get("s") == "ok" and "d" in response_data:
                ...
                if not order_id:
                    logger.warning(
                        f"place_order_api - No order ID in response: {response_data}"
                    )
                    return (
                        response_obj,
                ...
                logger.info(
                    f"place_order_api - Order placed successfully. Order ID: {order_id}"
                )
                return (
                    response_obj,
                ...
                error_msg = response_data.get("d", {}).get(
                    "msg", "Unknown error from broker"
                )
...
def place_smartorder_api(data, auth):
    ...
    try:
        ...
        try:
            ...
            return (
                None,
                {"status": "error", "message": f"Failed to get position: {str(e)}"},
                "",
            )
        ...
        # Case 2: No current position - create new position
            ...
            if position_size > 0:
                ...
                logger.info(
                    "place_smartorder_api - No position to create (position_size=0)"
                )
                return None, {"status": "success", "orderid": ""}, ""
        ...
        # Case 3: Adjusting existing position - position_size is the ABSOLUTE target position
            ...
            if position_size > current_position:
                ...
                logger.info(
                    "place_smartorder_api - Current position already matches target"
                )
                return None, {"status": "success", "orderid": ""}, ""
        ...
        if final_action is None or final_quantity <= 0:
            ...
            return (
                None,
                {"status": "error", "message": "No valid action determined"},
                None,
            )
    ...
    try:
        ...
        for position in positions:
            try:
                ...
                if net_quantity == 0:
                    logger.debug(
                        "close_all_positions - Skipping zero quantity position"
                    )
                    continue
...
def cancel_order(orderid, auth):
    ...
    try:
        ...
        if response["s"] == "ok":
            ...
            error_msg = (
                f"Order cancellation failed: {response.get('msg', 'Unknown error')}"
            )
            logger.error(f"cancel_order - {error_msg}")
            return {"stat": "Not_Ok", "data": {"msg": error_msg}}, 400
...
def cancel_all_orders_api(data, auth):
    ...
    try:
        ...
        logger.debug(
            f"cancel_all_orders_api - Order book response: {order_book_response}"
        )
        ...
        if order_book_response.get("stat") == "Ok":
            ...
            for order in orders:
                ...
                if status.upper() in ["OPEN", "TRIGGER PENDING", "MODIFIED", "PENDING"]:
                    logger.debug(
                        f"cancel_all_orders_api - Cancelling order: {order_id}"
                    )
...
def modify_order(data, auth):
    ...
    try:
        ...
        if response["s"] == "ok":
            ...
            error_msg = (
                f"Order modification failed: {response.get('msg', 'Unknown error')}"
            )
            logger.error(f"modify_order - {error_msg}")
            return {"stat": "Not_Ok", "data": {"msg": error_msg}}, 400
