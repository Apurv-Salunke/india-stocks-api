    ...
    try:
        ...
        if id_format == "instrument_symbol_series_exchange":
            ...
            if all([symbol, expiry, strike, opt_type]):
                strike_fmt = (
                    int(float(strike)) if float(strike).is_integer() else float(strike)
                )
            ...
            try:
                ...
                if len(parts) >= 3:
                    ...
                    record = {
                        ...
                        "brsymbol": item[
                            "dispName"
                        ],  # Use display name as broker symbol
            ...
            try:
                ...
                if group_name == "Securities":
                    ...
                    if openalgo_exchange == "BSE_INDEX":
                        ...
                        if (
                            item["symbol"].upper()
                            in COMMON_INDEX_MAP[openalgo_exchange]["common"]
                        ):
                            # Use common symbol format for main indices
                            record = {
                                "symbol": item[
                                    "symbol"
                                ].upper(),  # Use uppercase symbol
                            ...
                            record = {
                                ...
                                "brsymbol": COMMON_INDEX_MAP[openalgo_exchange][
                                    "sectoral"
                                ][item["symbol"].upper()],  # Use mapped brsymbol
                                "name": item.get(
                                    "symbol", item["dispName"]
                                ),  # Use symbol field if available
                                "exchange": openalgo_exchange,
                            ...
                            record = {
                                "symbol": item[
                                    "dispName"
                                ],  # Use dispName for non-common symbols
                                "brsymbol": item["id"],
                        ...
                        if (
                            item["symbol"].upper()
                            in COMMON_INDEX_MAP[openalgo_exchange]
                        ):
    ...
    try:
        ...
        for group in scrip_groups:
            try:
                ...
                logger.info(
                    f"Processing group: {group_name} (format: {group.get('idFormat')})"
                )
        ...
        if socketio:
            socketio.emit(
                ...
                {
                    "status": "success",
                    "message": "Successfully downloaded all contracts",
                },
            )
        return True
