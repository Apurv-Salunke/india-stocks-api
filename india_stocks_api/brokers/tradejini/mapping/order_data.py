...
def map_trade_data(trade_data):
    ...
    if (
        not isinstance(trade_data, dict)
        or "s" not in trade_data
        or trade_data.get("s") != "ok"
    ):
...
def transform_tradebook_data(trades):
    ...
    if not isinstance(trades, list):
        logger.error(
            f"Invalid input data type: Expected list or dict, got {type(trades)}"
        )
        return {
            "status": "error",
            "data": [],
    ...
    for trade in trades:
        ...
        if all(
            key in trade for key in ["action", "average_price", "exchange", "orderid"]
        ):
        ...
        transformed_trade = {
            "action": action,
            "average_price": float(
                trade.get("fillPrice", trade.get("average_price", 0.0))
            ),
    ...
    logger.debug(
        f"transform_tradebook_data - Transformed {len(transformed_trades)} trades"
    )
    ...
    for position in positions_data:
        try:
            ...
            transformed_position = {
                ...
                "quantity": quantity,
                "average_price": str(
                    round(float(position.get("average_price", 0.0)), 2)
                ),
            }
    ...
    for portfolio in portfolio_data:
        ...
        for exch_tsym in portfolio.get("exch_tsym", []):
            ...
            if symbol_from_db:
                ...
                logger.warning(
                    f"Zebu Portfolio - Product Value for {symbol} Not Found or Changed."
                )
...
def calculate_portfolio_statistics(holdings_data):
    ...
    totalpnlpercentage = (
        (totalprofitandloss / totalinvvalue) * 100 if totalinvvalue != 0 else 0
    )
        ...
        for holding in holdings_data:
            try:
                ...
                if investment_value > 0:
                    pnl_percent = (
                        (current_value - investment_value) / investment_value
                    ) * 100
