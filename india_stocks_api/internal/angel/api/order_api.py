...
from india_stocks_api.internal.angel.mapping.transform_data import (
    map_product_type,
...
def get_order_details(order_id, auth):
    return get_api_response(f"/rest/secure/angelbroking/order/v1/details/{order_id}", auth)


def get_profile(auth):
    return get_api_response("/rest/secure/angelbroking/user/v1/getProfile", auth)
...
def cancel_all_orders_api(data, auth):
    ...
    orders_to_cancel = [
        order for order in order_book_response.get("data", []) if order["status"] in ["open", "trigger pending"]
    ]
