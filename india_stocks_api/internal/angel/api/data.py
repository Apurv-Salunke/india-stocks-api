...
class BrokerData:
    ...
    def get_quotes(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            if not token:
                raise ValueError(
                    f"Could not resolve token for symbol '{symbol}' on exchange '{exchange}'. Symbol may not exist in the instrument database."
                )
            ...
            response = get_api_response("/rest/secure/angelbroking/market/v1/quote/", self.auth_token, "POST", payload)
    ...
    def _process_quotes_batch(self, symbols: list) -> list:
        ...
        for item in symbols:
            ...
            try:
                ...
                if not token:
                    ...
                    skipped_symbols.append({"symbol": symbol, "exchange": exchange, "error": "Could not resolve token"})
                    continue
        ...
        for key, original in token_map.items():
            ...
            if not quote:
                ...
                results.append(
                    {"symbol": original["symbol"], "exchange": original["exchange"], "error": "No quote data available"}
                )
                continue
            ...
            while current_start <= to_date:
                ...
                try:
                    response = get_api_response(
                        "/rest/secure/angelbroking/historical/v1/getCandleData", self.auth_token, "POST", payload
                    )
    ...
    def get_oi_history(self, symbol: str, exchange: str, interval: str, start_date: str, end_date: str) -> pd.DataFrame:
        """
        ...
        try:
            ...
            while current_start <= to_date:
                ...
                try:
                    response = get_api_response(
                        "/rest/secure/angelbroking/historical/v1/getOIData", self.auth_token, "POST", payload
                    )
