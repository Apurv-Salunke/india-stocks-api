...
from india_stocks_api.internal.context import get_auth_token, get_feed_token
...
class AngelWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def subscribe(self, symbol: str, exchange: str, mode: int = 2, depth_level: int = 5) -> Dict[str, Any]:
        ...
        if mode == 3 and depth_level not in [5]:
            return self._create_error_response("INVALID_DEPTH", f"Invalid depth level {depth_level}. Must be 5")
        ...
        token_list = [{"exchangeType": AngelExchangeMapper.get_exchange_type(brexchange), "tokens": [token]}]
        ...
        try:
            ...
            with self.lock:
                for sub in self.subscriptions.values():
                    if (
                        sub["token"] == token
                        and AngelExchangeMapper.get_exchange_type(sub["brexchange"]) == exchange_type
                    ):
                        subscription = sub
                        break
        ...
        if best_5_key in message and isinstance(message[best_5_key], list):
            ...
            for level in depth_data:
                if isinstance(level, dict):
                    ...
                    depth.append(
                        {"price": price, "quantity": level.get("quantity", 0), "orders": level.get("no of orders", 0)}
                    )
