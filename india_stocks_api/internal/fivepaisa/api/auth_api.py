...
def authenticate_broker(clientcode: str, broker_pin: str, totp_code: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    # Retrieve the credentials from context
    from india_stocks_api.internal.context import get_broker_creds
    creds = get_broker_creds()
    
    api_key = creds.get("app_key")
    user_id = creds.get("user_id")
    client_id = creds.get("client_code")
    api_secret = creds.get("api_secret")

    if not api_key or not user_id or not api_secret:
        return None, "Missing API credentials in context (api_key, user_id, api_secret)"
