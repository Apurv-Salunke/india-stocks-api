...
# Lazy loading of credentials from context
def _get_api_credentials():
    creds = get_broker_creds()
    return creds.get("app_key"), creds.get("user_id"), creds.get("client_code")
...
class BrokerData:
    ...
    def get_market_depth(self, symbol: str, exchange: str) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            # Prepare request payload
            json_data = _get_request_template()
            json_data["body"].update({
    ...
    def get_depth(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            # Get market snapshot for overall data
            snapshot_data = _get_request_template()
            snapshot_data["body"]["Data"] = [
                {
                    "Exchange": map_exchange(exchange),
                    "ExchangeType": map_exchange_type(normalized_exchange),
                    "ScripCode": token,
            ...
            depth_data = _get_request_template()
            depth_data["body"].update({
                "Exchange": map_exchange(exchange),
                "ExchangeType": map_exchange_type(normalized_exchange),
                "ScripCode": token,
                "ScripData": br_symbol if token == "0" else ""
    ...
    def _process_quotes_batch(self, symbols: list) -> list:
        ...
        json_data = _get_request_template()
        json_data["body"]["Data"] = data_array
