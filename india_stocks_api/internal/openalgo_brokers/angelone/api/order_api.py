import json
import os
from algotrade.internal.database.broker_instruments_db import get_broker_symbol, get_broker_token, get_standardized_symbol_by_token
from algotrade.internal.openalgo_brokers.angelone.mapping.transform_data import (
    transform_data,
    map_product_type,
    reverse_map_product_type,
    transform_modify_order_data,
)
from algotrade.internal.utils.httpx_client import get_http_client as get_httpx_client
...
def get_holdings(auth):
    return get_api_response(
        "/rest/secure/angelbroking/portfolio/v1/getAllHolding", auth
    )


def get_open_position(tradingsymbol, exchange, producttype, auth):
    # Convert Trading Symbol from OpenAlgo Format to Broker Format Before Search in OpenPosition
    tradingsymbol = get_broker_symbol(standardized_symbol=tradingsymbol, exchange_code=exchange, broker_name="angelone")
...
def place_order_api(data, auth):
    ...
    token =  get_broker_token(standardized_symbol=data["symbol"], exchange_code=data["exchange"], broker_name="angelone")
    logger.debug(f"Token for {data['symbol']} is {token}")
...
def close_all_positions(current_api_key, auth):
    ...
    if positions_response["status"]:
        ...
        for position in positions_response["data"]:
            ...
            symbol = get_standardized_symbol_by_token(broker_token=position["symboltoken"], exchange_code=position["exchange"], broker_name="angelone")
...
def modify_order(data, auth):
    ...
    token =  get_broker_token(standardized_symbol=data["symbol"], exchange_code=data["exchange"], broker_name="angelone")
    data["symbol"] = get_broker_symbol(standardized_symbol=data["symbol"], exchange_code=data["exchange"], broker_name="angelone")
